...
Chaque module doit déclarer `association_capacites` et
`association_installation_inventaire`. Une capacité porte un identifiant stable
et un champ `plugin` indiquant son fournisseur. Un pipeline métier public doit
être déclaré dans un `paquet.xml`, documenter la structure de son flux, rendre
un résultat stable en l'absence de contributeur et ne jamais imposer au
consommateur la lecture des tables d'un autre plugin.
...
| Adhésions/Cotisations | Inscription4, Intl, Médias | cotisations, profil membre, adaptateur Familles | sans Familles conserve un profil individuel; sans Compta conserve `id_compte=NULL`; sans Paiements accepte gratuit ou règlement manuel; sans Communication ignore l'envoi facultatif |
