...
function formulaires_configurer_association_saisies_dist($config = '', $lister_champs = '') {
	...
	$contributions = pipeline('association_configuration_saisies', [
		'args' => ['config' => $config, 'disable_meta_admin' => $disable_meta_admin],
		'data' => [],
	]);
	$contributions = is_array($contributions)
		? (array_key_exists('args', $contributions) ? ($contributions['data'] ?? []) : $contributions)
		: [];
	usort($contributions, function ($a, $b) { return $a['ordre'] <=> $b['ordre']; });
	foreach ($contributions as $contribution) {
		$saisies = array_merge($saisies, (array) ($contribution['saisies'] ?? []));
	}
			...
			'saisies' => [
				[
					...
					'options' => [
						...
						'data' => ['oui' => _T('association_config:oui'), 'non' => _T('association_config:non')],
						'defaut' => 'oui',
					],
				],
				[
			...
			foreach (association_log_categories_defaut() as $cat => $libelle) {
				$saisies_debug[] = [
					...
					'options' => [
						...
						'label' => _T('association_config:config_debug_cat_' . $cat . '_label', [], $libelle),
						'label_case' => _T('association_config:config_debug_cat_' . $cat . '_label', [], $libelle),
						'explication' => _T('association_config:config_debug_cat_' . $cat . '_explication', [], ''),
					],
				];
			}
...
function formulaires_configurer_association_verifier_dist($config) {
	...
	$contributions = pipeline('association_configuration_verifier', [
		'args' => ['config' => $config],
		'data' => [],
	]);
	$contributions = is_array($contributions)
	...
	usort($contributions, function ($a, $b) { return $a['ordre'] <=> $b['ordre']; });
	foreach ($contributions as $contribution) {
		$erreurs = array_merge($erreurs, (array) ($contribution['erreurs'] ?? []));
	}
	...
	$erreurs = pipeline('association_maintenance_bdd_verifier_configuration', [
		'args' => ['config' => $config],
		'data' => $erreurs,
	]);
...
function formulaires_configurer_association_traiter_dist($config) {
	...
	$trace = cvtconf_configurer_stocker('configurer_association', ['_meta_table' => 'association_metas'], $valeurs);
	...
	if (isset($_REQUEST['exec_maintenance_dry_run']) || _request('exec_maintenance_dry_run')) {
		...
		if (empty($GLOBALS['visiteur_session']) || ($GLOBALS['visiteur_session']['webmestre'] != 'oui')) {
			...
			$retours['message_ok'] .= '<br/>' . _T('association_config:config_maintenance_bdd_exec_dry_run_done', ['fichier' => basename($chemin)]);
