...
foreach ($modules_autonomes as $module_autonome) {
	...
	$verifier(!preg_match('/<necessite nom="association_(?:adhesions|communication|compta|dons|evenements|groupes|paiements|prets|ventes|commerce|partenaires|bannieres|bons_plans)"/', $paquet_module), "$module_autonome ne doit necessiter aucun autre module metier.");
	$verifier(strpos($paquet_module, 'nom="association_capacites"') !== false, "$module_autonome doit publier ses capacites par le pipeline commun.");
	$verifier(strpos($paquet_module, 'nom="association_installation_inventaire"') !== false, "$module_autonome doit contribuer au contrat d installation autonome.");
}

foreach (array(
	'association-bannieres' => 'association_bannieres',
	'association-commerce' => 'association_commerce',
	'association-partenaires' => 'association_partenaires',
	'association-bons-plans' => 'association_bons_plans',
) as $module => $prefixe) {
	$pipelines = file_get_contents($racine . '/plugins/' . $module . '/' . $prefixe . '_pipelines.php');
	$verifier(
		strpos($pipelines, "\$capacites['" . str_replace('association_', '', $prefixe) . "']") !== false
			&& strpos($pipelines, "'plugin' => '" . $prefixe . "'") !== false,
		"$module doit publier une capacite indexee et identifier son plugin fournisseur."
	);
}
