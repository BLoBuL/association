...
try {
	...
	if ($code !== 0) {
		...
		if (count($packages) !== 14) {
			$erreurs[] = 'le staging doit contenir exactement quatorze paquet.xml de premier niveau';
		}
		...
		if (!is_array($manifest) || count($manifest['plugins'] ?? array()) !== 14) {
...
echo "OK: staging du monorepo en quatorze plugins freres.\n";
