...
function action_test_notification_cotisation_dist() {
    ...
    list($notification, $type, $lang, $delai, $id_compte, $id_auteur, $email) = array_pad(explode(':', $arg, 7), 7, '');
    ...
    if (strpos($notification, 'echeances') !== false) {
        ...
        if (strpos($notification, 'admin') !== false) $target_admin = true;
    ...
    if ($type_map) {
        ...
        if ($target_admin) {
                ...
                if (!empty($email)) {
                    ...
                    if (!empty($parsed)) {
                        ...
                        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            $opts['email_override'] = array($email);
                        }
                    }
            ...
            $opts = array('use_queue' => false);
            if ($delai !== null) $opts['nb_jour_differences'] = intval($delai);
            if (!empty($lang)) $opts['lang'] = $lang;
            if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) $opts['email_override'] = $email;
