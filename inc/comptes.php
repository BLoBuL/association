...
function modifier_compte(
	...
	if ($reinscription !== null || $statut_cotisation !== null) {
		include_spip('inc/cotisations_stockage');
		association_cotisation_synchroniser_depuis_compte($id_compte_int, array_merge($update, array(
 ...
 * @param float $montant_choisi Montant de la cotisation
 * @param string $justification Description de l'opération
 * @param string $imputation Code d'imputation comptable
 * @param string $journal Journal comptable
 * @param int $id_auteur ID de l'adhérent
 * @param string $inscription Type d'inscription (inscription/réinscription)
 * @param int $id_categorie Catégorie d'adhérent
 * @param string $statut_cotisation Statut de la cotisation
 * @param int $id_transaction ID de la transaction associée
 * @return int ID du compte créé
 */
function compte_cotisation(
    $date,
    $montant_choisi,
    $justification,
    $imputation,
    ...
    // Correction du problème de décalage des paramètres
    return inserer_compte(
        $date,                  // date
        $montant_choisi,        // recette
        0,                      // depense
        $justification,         // justification
        $imputation,            // imputation
        $journal,               // journal
        ...
        null,                   // id_objet
        'cotisation',           // objet
        $reinscription,           // inscription (reinscription)
        $id_categorie,          // id_categorie
        $statut_cotisation,     // statut_cotisation
        $id_transaction,        // id_transaction
        create_destination_map_for_montant('dc_cotisations', $montant_choisi) // destination_map
    );
}
 ...
 *
 * @param string $date Date de l'opération
 * @param float $montant Montant de la cotisation
 * @param string $justification Description de l'opération
 * @param string $imputation Code d'imputation comptable
 * @param string $journal Journal comptable
 * @param string $reinscription Type d'inscription (inscription/réinscription)
 * @param int $id_categorie Identifiant de la catégorie
 * @param int $id_compte Identifiant du compte à modifier
 * @param string $statut_cotisation Statut de la cotisation
 * @param int $id_transaction Identifiant de la transaction associée
 *
 * @return void
 */
function modifier_compte_cotisation(
    $date,
    $montant,
    $justification,
    $imputation,
