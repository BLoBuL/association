...
function association_bilan_contexte($inutile = '') {
	...
	$ids = array_values(array_unique(array_map('intval', (array) (_request('destination') ?: [0]))));
	...
	foreach ($ids as $id_destination) {
		...
		$where = ['C.date>=' . sql_quote($annee . '-01-01'), 'C.date<' . sql_quote(($annee + 1) . '-01-01')];
		$group = 'C.imputation';
		if ($id_destination) {
		...
		$totaux = ['recettes' => 0.0, 'depenses' => 0.0, 'solde' => 0.0];
	...
	if ($avec_plan && $classe_banques !== '') {
		foreach (sql_allfetsel('*', 'spip_asso_plan', 'classe=' . sql_quote($classe_banques), '', 'code') as $banque) {
			...
			$mouvements = sql_fetsel('SUM(recette) AS recettes,SUM(depense) AS depenses', 'spip_asso_comptes', ['date>=' . sql_quote($banque['date_anterieure']), 'journal=' . sql_quote($banque['code'])]);
			$actuel = $initial + (float) ($mouvements['recettes'] ?? 0) - (float) ($mouvements['depenses'] ?? 0);
			$total_initial += $initial;
			$total_actuel += $actuel;
			$encaisses[] = ['intitule' => $banque['intitule'], 'date' => $banque['date_anterieure'], 'initial' => $initial, 'actuel' => $actuel];
		}
	}
