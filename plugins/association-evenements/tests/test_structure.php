...
$erreurs = [];
foreach (['prefix="association_evenements"', 'schema="1.2.0"', 'nom="agenda"', 'nom="saisies"', 'nom="verifier"'] as $attendu) {
	if (strpos($paquet, $attendu) === false) {
		$erreurs[] = 'déclaration absente: ' . $attendu;
	}
}
foreach (['spip_asso_categories_activites', 'spip_asso_activites', 'spip_asso_categories_activites_liens'] as $table) {
	if (strpos($base, $table) === false) {
		$erreurs[] = 'table absente: ' . $table;
	}
}
foreach (['type_inscrit', 'association', 'participants_json', 'visible_in_list_members', 'notify_the_members', 'journal', 'annotation', 'condition_inscription'] as $champ) {
	if (strpos($base, "'" . $champ . "'") === false) {
		$erreurs[] = 'champ métier absent: ' . $champ;
	}
}
if (strpos($admin, 'sql_drop_table') !== false) {
	$erreurs[] = 'désinstallation destructive';
}
...
foreach (['inc/actions', 'inc/editer', 'inc/autoriser'] as $api) {
	if (!str_contains($fonctions, "include_spip('$api')")) {
		$erreurs[] = "la surcharge Agenda doit charger l’API SPIP $api";
	}
}
if (!is_file($racine . '/squelettes/evenement.html')) {
	$erreurs[] = 'page publique événement absente';
}
if ($erreurs) {
