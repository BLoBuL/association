...
function association_evenements_association_capacites($capacites) {
	$capacites['evenements'] = ['plugin' => 'association_evenements'];
	$capacites['inscriptions_evenements'] = ['plugin' => 'association_evenements'];
...
function association_evenements_association_paiements_reglement_traiter($flux) {
	...
	association_evenements_reglement_traiter($activite, $flux['args']['transaction'] ?? []);
	$flux['data'] = ['traite' => true, 'domaine' => 'evenements'];
	return $flux;
}
...
function association_evenements_association_compta_autoriser_ecriture($flux) {
	...
	$opt = is_array($flux['args']['opt'] ?? null) ? $flux['args']['opt'] : [];
...
function association_evenements_association_configuration_saisies($flux) {
	...
	$flux['data'][] = ['ordre' => 30, 'saisies' => association_evenements_configurer_saisies(
		$flux['args']['config'] ?? '',
...
function association_evenements_association_maintenance_bdd_executer($flux) {
	...
	$inactifs = (array) ($flux['args']['inactifs'] ?? []);
	...
	$flux['data']['inscriptions_non_validees_anciennes'] = [
	...
	if (!empty($actions['supprimer_inscriptions_non_validees']) && $anciennes) {
		...
		if (association_maintenance_resultat_en_echec($transactions)) {
			$flux['data']['supprimer_inscriptions_non_validees'] = ['skipped' => true, 'raison' => 'suppression_transactions_echec'];
		} else {
			$flux['data']['supprimer_inscriptions_non_validees'] = asso_supprimer_inscriptions_par_ids(
				$transactions['ids_activite_supprimables'] ?? [],
				$dry_run
			);
		}
	} else {
		$flux['data']['supprimer_transactions_inscriptions'] = ['skipped' => true];
		$flux['data']['supprimer_inscriptions_non_validees'] = ['skipped' => true];
	}
	if ($inactifs) {
...
function association_evenements_association_rgpd_anonymiser_auteur($flux) {
	...
	$flux['data']['activites_anonymisees'] = association_rgpd_updateq('spip_asso_activites', association_rgpd_filtrer_champs('spip_asso_activites', [
...
function association_evenements_champs_extras_dedoublonner_afficher_si($saisies, $condition_parent = '') {
	foreach ($saisies as $cle => $saisie) {
		...
		foreach (($saisie['options'] ?? []) as $option => $valeur) {
...
function association_evenements_normaliser_requete() {
	foreach (['mode_paiement', 'responsables'] as $champ) {
...
function association_evenements_post_insertion($flux) {
	...
	while ($lien = sql_fetch($liens)) {
		if ($lien['montant'] !== '' && $lien['montant'] !== null) {
			sql_insertq('spip_asso_categories_activites_liens', ['id_evenement' => $id_evenement, 'id_categorie' => $lien['id_categorie'], 'montant' => $lien['montant']]);
		}
	}
	return $flux;
}
...
function association_sync_repetitions_tarifs($id_evenement) {
	...
	foreach ($repetitions as $rep_row) {
		...
		$to_insert = []; // cat => montant
		$to_update = []; // cat => montant
		$to_delete = []; // list of cat
