...
function ie_multi_public_contexte_cvt_precedent() {
	...
	$normalise = ie_convertir_post(['cvtm_prev_post' => $token]);
...
function ie_multi_public_saisies($id_evenement, $id_activite = null) {
	...
	$famille_selectionnee = !empty($GLOBALS['visiteur_session']['id_auteur'])
		? ie_aplatir_liste_valeurs(_request('famille') ?: ['adherent'])
	...
	if (isset($GLOBALS['visiteur_session']['id_auteur'])) {
		...
		$data_famille = $info_auteur_connecte['data_famille'] ?? [];
	...
	if (!empty($query_article_CGU) && !$id_auteur_connecte) {
		$saisies_modalites['saisies'][] = [
			...
			'options' => [
				...
				'label_case' => _T('association_evenements:message_conditions_generales', ['url_CGU' => $url_CGU, 'titre_CGU' => $titre_CGU]),
				'obligatoire' => 'oui',
			],
		];
	}
...
function formulaires_inscription_evenement_multi_public_saisies($id_evenement, $id_activite) {
	[$id_evenement, $id_activite] = ie_inscription_multi_public_ids($id_evenement, $id_activite);
...
function ie_multi_public_verifier_legacy($id_evenement, $id_activite = null, $etape_validation = null) {
	include_spip('formulaires/inc/inscription_evenement_backend');
	return ie_verifier_commons('multi_public', $id_evenement, $id_activite, [], $etape_validation);
}
/**
