...
function formulaires_desinscription_evenement_public_charger_dist($id_evenement) {
	...
	while ($asso_activite = sql_fetch($query_activite)) {
		...
		$id_auteur_inscrit = $asso_activite['id_auteur'] ?? '';
	...
	if (isset($id_activite)) {
		...
		$valeurs = ['desinscription_possible' => 'pas_inscrit'];
	}
	return $valeurs;
}
...
function formulaires_desinscription_evenement_public_traiter_dist($id_evenement) {
	...
	job_queue_add('facteur_envoyer_mail_activites', 'Notification - desinscription_frontend - Activite :' . $id_activite . '', $arguments = [$id_evenement, $type = 'desinscription_frontend',
		[$id_activite]], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
	...
	if (($affichage_dans_activites['validation_attente_automatique'] ?? '') == 'oui') {
		...
		job_queue_add('validation_attente_automatique', 'Validation automatique après désinscription- Activite :' . $id_activite . '', $arguments = [$id_evenement, 'form_desinscription_public'], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
	}
