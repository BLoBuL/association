 ...
 */
function association_evenements_inscription_debug($code, $contexte = []) {
...
function association_evenements_inscription_debug_message($message, $canal = '') {
	association_evenements_inscription_debug('diagnostic', ['message' => (string) $message]);
}
...
function preparer_info_auteur($id_auteur, $id_evenement) {
	...
	$profil = association_evenements_profil_participant(['id_auteur' => (int) $id_auteur]);
	$res['statut_interne_auteur'] = !empty($profil['est_membre']) ? 'ok' : '';
	$res['radio_type_adherent'] = $query_auteur['radio_type_adherent'] ?? false;
 ...
 */
function generer_array_categories_participation($tableau_categories, $format = '', $types = ['adherent'], $rang = 1, $contexte = []) {

	// Initialisation des tableaux pour stocker les résultats
	$datas_categories = $saisie_categories = $datas_categories_tous = $saisie_categories_tous = [];
...
function comparer_modification($query_activite, $donnees_saisies, $ancienne_transaction, $nouvelle_transaction) {
	...
	if ($query_activite['nombre_inscrits'] < $donnees_saisies['nombre_inscrits']) {
		...
		$modification['nombre_inscrits'] = ['difference' => $difference];
	...
	if ($query_activite['nom_participants'] != $donnees_saisies['nom_participants']) {
		...
		$modification['participants'] = ['difference' => $difference];
	}
...
function formater_post_form($id_evenement, $valeurs_post, $affichage_dans_activites, $public_or_prive = '') {
	...
	$config_accompagnants = $GLOBALS['association_metas']['meta_cfg_event_config_accompagnants'] ?? false;
	...
	association_evenements_inscription_debug('formater_input', [
		'trace' => $trace_id,
	...
	if ($public_or_prive == 'public') {
		$id_auteur = $GLOBALS['visiteur_session']['id_auteur'] ?? false;
	...
	if ($affichage_dans_activites['payant'] == false) {
		if ($saisie_famille_active == 'oui') {
			$request_famille = isset($data_form['famille']) && is_array($data_form['famille']) ? $data_form['famille'] : [$data_form['famille']];
		...
		$categorie_result = $data_form['categorie'] ?? false;
		...
		if (!is_array($categorie_result) && !empty($categorie_result)) {
			$categorie_result = [intval($categorie_result) => 1];
		} elseif (empty($categorie_result)) {
			$categorie_result = false;
		}

		association_evenements_inscription_debug('formater_categorie_normalisee', [
			'trace' => $trace_id,
		...
		if (is_array($categorie_result) && !empty($categorie_result)) {
			...
			while ($categories_activite = sql_fetch($querie_categorie_activite)) {
				...
				if (isset($categorie_result[$id_categorie])) {
					...
					association_evenements_inscription_debug_message('[IE_FORMATER_CATEGORIE_CALCUL][' . $trace_id . '] ' . json_encode([
			...
			if (empty($transaction) && ($saisie_famille_active == 'oui' || $saisie_famille_effective)) {
				foreach ($categorie_result as $id_categorie => $vals) {
					if (is_array($vals)) {
						...
						association_evenements_inscription_debug_message('[IE_FORMATER_CATEGORIE_CALCUL_FALLBACK][' . $trace_id . '] ' . json_encode([
							'id_categorie' => intval($id_categorie),
						...
						$nb = intval($vals);
						$nombre_participants += $nb;
						$transaction[$id_categorie] = ['nombre' => $nb, 'montant' => 0, 'id_participants' => []];
					}
				}
			}
		} else {
			...
			$nombre_participants = 1;
			$transaction = [];
			association_evenements_inscription_debug_message('[IE_FORMATER_CATEGORIE_VIDE][' . $trace_id . '] ' . json_encode([
				'id_evenement' => intval($id_evenement),
	...
	$id_participants = is_array($id_participants) ? $id_participants : [];
	...
	association_evenements_inscription_debug('formater_output', [
		'trace' => $trace_id,
...
function ie_aplatir_liste_valeurs($valeur) {
	if (!is_array($valeur)) {
		return ($valeur === '' || $valeur === null) ? [] : [$valeur];
	}
...
function formater_post_form_multi($valeurs_post, $public_or_prive = '') {
			...
			foreach ($valeurs_post as $cle_post => $valeur_post) {
				if (strpos($cle_post, '_' . $membre_famille) !== false && strpos($cle_post, 'fieldset') == false) {
					...
					if (!isset($array_post_inscrits[$membre_famille])) {
						$array_post_inscrits[$membre_famille] = [];
					}
	...
	foreach ($valeurs_post as $cle_post => $valeur_post) {
		if (strpos($cle_post, '_inscrit_') !== false && strpos($cle_post, 'fieldset') == false) {
			...
			if (!isset($array_post_inscrits["inscrit_$id_inscrit"])) {
				$array_post_inscrits["inscrit_$id_inscrit"] = [];
			}
			// Stocke les données dans le tableau regroupé
		...
		} elseif (in_array($cle_post, ['commentaire', 'annotation', 'notifier'])) {
	...
	if (!empty($categorie_result)) {
		$res['categorie_result'] = is_array($categorie_result) ? $categorie_result : [$categorie_result];
	}
	...
	if (empty($res['categorie_result']) && isset($valeurs_post['categorie']) && !empty($valeurs_post['categorie'])) {
		...
		if (is_array($posted_cat)) {
			$map = [];
			$nb = 0;
			$filtre_famille = !empty($valeur_post_famille) ? array_flip($valeur_post_famille) : [];
	...
	$res['nombre_participants_reels'] = count((array) ($res['array_post_inscrits'] ?? []));
...
function analyser_selection_tarifs_evenement($id_evenement, $categorie_result) {
	...
	if (empty($tarifs)) {
		...
		foreach ((array) ($affichage['montant'] ?? []) as $tarif) {
	...
	$selection = is_array($categorie_result) ? $categorie_result : [];
	$transaction = [];
	$invalides = [];
...
function ie_construire_participants_json($data_form) {
	$array_inscrits = is_array($data_form['array_post_inscrits'] ?? null) ? $data_form['array_post_inscrits'] : [];

	if (!empty($data_form['participants_json'])) {
		// Mode famille : base = membres famille, on fusionne les inscrit_N (invités)
		$base = json_decode($data_form['participants_json'], true) ?: [];
...
function inserer_asso_activites($id_evenement, $data_form, $cal_result, $id_transaction, $nouvelle_transaction, $message_journal, $ip_client = '') {
	$premier_inscrit = isset($data_form['premier_inscrit']) && is_array($data_form['premier_inscrit']) ? $data_form['premier_inscrit'] : [];

	/* CREATION DE LA LIGNE DANS LA BDD */
	$id_activite = sql_insertq('spip_asso_activites', [
...
function modifier_asso_activites($id_activite, $data_form, $cal_result, $id_transaction, $nouvelle_transaction, $message_journal, $ip_client = '') {
	...
	$id_activite = sql_updateq('spip_asso_activites', [
...
function notifier_inscription_activite($id_activite, $id_evenement, $cal_result_statut, $prive_ou_public = 'prive') {
	...
	return job_queue_add('facteur_envoyer_mail_activites', 'Notification - ' . $type . ' - ' . $id_activite, $arguments = [$id_evenement, $type, [$id_activite]], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
}
/**
...
function inserer_transaction_activites($montant_total, $id_auteur) {
	...
	$pourcentage_taxe = $GLOBALS['association_metas']['meta_cfg_taxe_evenement'] ?? false;
	if ($pourcentage_taxe) {
...
function verifier_spam_formulaire_inscription($post_data, $email_field = 'email_inscrit', $prenom_field = 'prenom_inscrit', $nom_field = 'nom_inscrit', $check_identical_names = true, $id_auteur = 0) {
	...
	foreach ($keys as $k) {
		if (!empty($post_data[$k])) {
			...
			association_evenements_inscription_debug('spam_nobot', ['champ_present' => 'oui']);
			break;
		}
	}
	...
	if (isset($post_data[$email_field])) {
		...
		$default_blacklist = ['qq.com', 'mailbox.in.ua'];
		$blacklist_meta = $GLOBALS['association_metas']['meta_cfg_spam_email_blacklist'] ?? '';
		...
		foreach ($blacklist as $blocked) {
			if ($blocked && $email_domain == strtolower($blocked)) {
				$erreurs['message_erreur'] = _T('pass_rien_a_faire_ici');
				association_evenements_inscription_debug('spam_domaine_email', ['domaine_bloque' => 'oui']);
				break;
			}
		}
	}
	...
	if ($check_identical_names && !$id_auteur && isset($post_data[$prenom_field]) && isset($post_data[$nom_field]) && $post_data[$prenom_field] == $post_data[$nom_field]) {
		$erreurs['message_erreur'] = _T('pass_rien_a_faire_ici');
		association_evenements_inscription_debug('spam_identite_identique', ['identite_presente' => 'oui']);
	}
...
function inscrire_participant_mailsubscriber($data_form) {
	...
	$options['force'] = true;
	$options['listes'] = ['liste_inscrit_activite'];
...
function ie_creer_liste_diffusion_activite_si_absente($identifiant_liste = 'liste_inscrit_activite') {
	...
	if ($id_liste) {
		objet_modifier('mailsubscribinglist', intval($id_liste), [
