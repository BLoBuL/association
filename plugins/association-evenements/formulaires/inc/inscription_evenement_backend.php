...
// Backend commun d'orchestration des parcours charger/verifier/traiter.
...
function ie_resoudre_ids_inscription($id_evenement = 0, $id_activite = null) {
	...
	return [$id_evenement, $id_activite > 0 ? $id_activite : null];
}
...
function ie_log_formater_multi($trace_id, $mode, $post, $data_form, $phase) {
	...
	$premier_inscrit = is_array($data_form['premier_inscrit'] ?? null)
		...
		: [];
	$champs_simples = ['prenom_inscrit', 'nom_inscrit', 'email_inscrit', 'tel_inscrit'];
	$champs_reconstruits = [];
	foreach (['prenom', 'nom', 'email', 'tel'] as $champ) {
	...
	foreach ((array) ($data_form['categorie_result'] ?? []) as $qte) {
		$categories_selectionnees += is_array($qte)
			? count(array_filter($qte))
			: max(0, intval($qte));
	}

	association_evenements_inscription_debug_message('[IE_FORMATER_MULTI][' . $trace_id . '] ' . json_encode([
		'phase' => (string) $phase,
		'mode' => (string) $mode,
 ...
 * @return array
 */
function ie_charger_commons($mode, $id_evenement = 0, $id_activite = null, $opts = []) {
	[$id_evenement, $id_activite] = ie_resoudre_ids_inscription($id_evenement, $id_activite);
	// init
	$res = [];
	$saisies = [];
	$saisies_general = [];
	...
	$est_mode_bo = in_array($mode, ['prive', 'multi_prive']);
	if (!$est_mode_bo) {
	...
	if ($id_activite) {
		...
		if ($mode_multi) {
			...
			for ($j = 1; $j <= $nb_invite_stored; $j++) {
				$cle = 'inscrit_' . $j;
				$champs_invite = ['prenom', 'nom', 'email', 'telephone', 'date_naissance',
	...
	$config_accompagnants = $GLOBALS['association_metas']['meta_cfg_event_config_accompagnants'] ?? 'tout';
	...
	if (in_array($mode, ['prive', 'multi_prive'])) {
		$liste_membre = [];
		$liste_non_membre = [];
		$ids_auteurs_deja_inscrits = [];
	...
	for ($i = 2; $i <= max(2, $max_places_limit); $i++) {
		$nb_personne[$i] = _T('association_evenements:activite_form_public_nb_personnes', ['nb' => $i]);
	}
	...
	if (isset($affichage_dans_activites['payant']) && $affichage_dans_activites['payant'] == true) {
		...
		if (($statut_interne_auteur_connecte ?? '') == 'ok' && in_array($statut_auteur ?? '', ['1comite', '0minirezo'])) {
		...
		if ($saisie_famille_active == 'oui' && !empty($id_auteur_connecte) && empty($affichage_dans_activites['accompagnants'])) {
			...
			association_evenements_inscription_debug('charger_famille_payant', [
				'id_auteur' => intval($id_auteur_connecte),
				'nombre_membres' => count((array) $data_famille),
			]);
			$datas_categories_filtrees = generer_array_categories_participation($tableau_categories, 'datas', $array_type_adherents, 1);
			if (empty($datas_categories_filtrees)) {
				$datas_categories_filtrees = $datas_categories;
			...
			foreach ($data_famille as $cle_membre => $prenom_membre) {
				...
				$saisies_tarifs[] = ['saisie' => $type_saisie_tarif, 'options' => $opts];
			}
			...
			foreach ($saisie_categories as $saisie_categorie) {
				...
				$saisies_tarifs[] = ['saisie' => 'selection', 'options' => $opts];
			}
	...
	if ($id_evenement > 0) {
		$saisies_hidden[] = [
			'saisie' => 'hidden',
			'options' => ['nom' => 'id_evenement', 'defaut' => $id_evenement],
		];
	}
	...
	if ($id_activite) {
		$saisies_hidden[] = [
			'saisie' => 'hidden',
			'options' => ['nom' => 'id_activite', 'defaut' => $id_activite],
		];
		$saisies_hidden[] = [
			'saisie' => 'hidden',
			'options' => ['nom' => 'modif', 'defaut' => 'oui'],
		];
	...
	$flux_charger = pipeline('association_inscription_evenement_charger', [
		'args' => [
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
			...
			'post' => ie_requete_attendue($mode, $id_evenement, $id_activite, []),
		],
		'data' => [
	...
	if ($multi_etapes) {
		...
		if ($saisie_famille_active == 'oui' && $id_auteur_connecte) {
			...
			$saisies_inscrits = champs_saisies_inscrits($affichage_dans_activites, $nb_inscrits, ['id_auteur' => $id_auteur_connecte]);
	...
	if ($multi_etapes) {
		$res = array_merge($res, ['_saisies_par_etapes' => $saisies_par_etapes]);
		association_evenements_inscription_debug('charger_multi', [
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
			'id_activite' => intval($id_activite),
			'nombre_saisies' => count($saisies),
			'nombre_etapes' => count($saisies_par_etapes),
		]);
	}
	...
	if (!empty($data_activite)) {
		$res = array_merge($data_activite, $res, ['id_activite' => $id_activite]);
	}
...
function ie_types_tarifs_auteur($id_auteur) {
	$types = ['non_adherent', 'indifferent'];
	...
	$types = ['adherent', 'indifferent'];
	if (in_array($auteur['statut'] ?? '', ['1comite', '0minirezo'], true)) {
		$types[] = 'benevole';
	}
 ...
 * @return array
 */
function ie_normaliser_tarifs_postes($mode, $post, $data_form, $ids_categories_evenement = []) {
	$selections = [];
	$mode_multi = (strpos($mode, 'multi') !== false);

	if ($mode_multi) {
		foreach ((array) ($data_form['categorie_result'] ?? []) as $participant => $id_categorie) {
	...
	$roles = ['adherent', 'conjoint', 'enfant_1', 'enfant_2', 'enfant_3', 'enfant_4', 'enfant_5', 'invite_1', 'invite_2'];
...
function ie_verifier_tarifs_compatibles($id_evenement, $mode, $post, $data_form) {
	...
	if (in_array($select_type_inscrit, ['public', 'membre_reseau_fiafe'], true)) {
		$id_auteur_tarifs = 0;
	}
	$types_auteur = ie_types_tarifs_auteur($id_auteur_tarifs);
	$participants = array_keys((array) ($data_form['array_post_inscrits'] ?? []));
	if (empty($participants)) {
		$participants = ie_aplatir_liste_valeurs($post['famille'] ?? []);
	}
	...
	foreach ($selections as $selection) {
		...
		if (!$categorie) {
			return [$cle_erreur => _T('association_evenements:erreur_tarif_incompatible')];
		}

		if ($type === 'couple') {
			$premier = in_array($participant, ['adherent', 'inscrit_1'], true);
...
function ie_verifier_commons($mode, $id_evenement = 0, $id_activite = null, $post = null, $etape_validation = null) {
	...
	$post = ie_requete_attendue($mode, $id_evenement, $id_activite, is_array($post) ? $post : []);
	...
	association_evenements_inscription_debug_message('[IE_VERIFY][' . $trace_id . '] sources mode_multi=' . ($mode_multi ? 'oui' : 'non')
	...
	association_evenements_inscription_debug_message('[IE_VERIFY][' . $trace_id . '] calcul nombre_inscrits=' . intval($nombre_inscrits), $log_canal);
	...
	if ($famille_a_verifier && !$mode_multi) {
		$id_participants_verif = $data_form['id_participants'] ?? [];
		...
		if ($email_multi_public !== '') {
			...
			if (!empty($query_activite_email)) {
				...
				if (!empty($query_auteur_email['id_auteur'])) {
					$erreurs['email_inscrit_1'] = _T('association_evenements:erreur_email_inscrit_adherent', [
	...
	if (!$mode_multi
		...
		association_evenements_inscription_debug_message('[IE_VERIFY_NOM_PARTICIPANTS][' . $trace_id . '] ' . json_encode([
			'mode' => $mode,
	...
	$flux_verifier = pipeline('association_inscription_evenement_verifier', [
		'args' => [
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
			'id_activite' => intval($id_activite),
	...
	association_evenements_inscription_debug_message('[IE_VERIFY][' . $trace_id . '] quotas nombre_a_verifier=' . intval($nombre_a_verifier)
	...
	if (empty($erreurs)) {
		...
		association_evenements_inscription_debug_message('[IE_VERIFY_RETURN_NONE][' . $trace_id . '] aucune_erreur_detectee', $log_canal);
		return [];
	}
	...
	if (!empty($erreurs)) {
		...
		association_evenements_inscription_debug_message('[IE_VERIFY_BLOCK][' . $trace_id . '] ' . json_encode($payload_debug), 'association' . _LOG_DEBUG);
		association_evenements_inscription_debug_message('[IE_VERIFY_RETURN][' . $trace_id . '] retour=' . (empty($erreurs) ? 'OK' : 'KO_' . count($erreurs) . '_erreurs'), 'association' . _LOG_DEBUG);
	}

	association_evenements_inscription_debug_message('[IE_VERIFY_RETURN_FINAL][' . $trace_id . '] ' . json_encode([
		'mode' => $mode,
		'id_evenement' => intval($id_evenement),
	...
	if (!empty($affichage_dans_activites['invites'])) {
		...
		if ($nb_invite_soumis > $limite_invites) {
			$erreurs['nb_invite'] = _T('association_evenements:erreur_nb_invite_depasse', ['max' => $limite_invites]);
		} else {
	...
	if (association_diagnostic_inscription_webmestre() && !empty($motifs_nb_inscrits)) {
		$non_field_messages[] = 'Diagnostic validation — ' . ie_diagnostic_validation_code([
	...
	association_evenements_inscription_debug('verifier_resultat', [
		'trace' => $trace_id,
		'erreurs' => array_keys($consolidated),
	]);

	return $consolidated;
}

function ie_nombre_inscrits_contexte($data_activite = []) {
	...
	if ($nb_inscrits <= 0) {
		...
		if (!is_array($precedentes) && $precedentes !== null && $precedentes !== '') {
			$post_normalise = ie_convertir_post(['cvtm_prev_post' => $precedentes]);
			$precedentes = $post_normalise['cvtm_prev_post'] ?? [];
		}
...
function ie_diagnostic_validation_code($valeurs) {
	$motifs = array_values(array_unique(array_filter((array) ($valeurs['motifs'] ?? []))));
	return implode('-', [
...
function ie_traiter_commons($mode, $id_evenement = 0, $id_activite = null, $post = null) {
	...
	$flux_traiter_pre = pipeline('association_inscription_evenement_traiter', [
		'args' => [
			'etape' => 'avant',
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
	...
	$analyse_tarifs = $is_payant_traitement
		? analyser_selection_tarifs_evenement($id_evenement, $data_form['categorie_result'] ?? [])
		: ['valide' => true, 'nombre_participants' => intval($data_form['nombre_participants'] ?? 0)];
	...
	if ($nombre_participants <= 0 || $nom_premier_inscrit === '' || $quantite_incoherente) {
		association_evenements_inscription_debug_message('[IE_TRAITER_BLOCK][' . $trace_id . '] ' . json_encode([
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
	...
	association_evenements_inscription_debug('traiter_input', [
		'trace' => $trace_id,
		'mode' => $mode,
		'id_evenement' => intval($id_evenement),
		'id_activite' => intval($id_activite),
		'nombre_participants' => intval($data_form['nombre_participants'] ?? 0),
		'categorie_count' => is_array($data_form['categorie_result'] ?? null) ? count($data_form['categorie_result']) : 0,
		'montant_total_data_form' => floatval($data_form['montant_total'] ?? 0),
		'transaction_count' => is_array($data_form['transaction'] ?? null) ? count($data_form['transaction']) : 0,
	]);
	...
	if ($is_payant) {
		...
		if (!empty($id_activite)) {
			...
			if (intval($activite_existante['id_evenement'] ?? 0) !== intval($id_evenement)) {
				return ['editable' => true, 'message_erreur' => _T('association_evenements:erreur_paiement_inscription_incoherent')];
			}
		...
		$nouvelle_transaction = serialize($calc_tr['transaction'] ?? ($data_form['transaction'] ?? []));
		association_evenements_inscription_debug('traiter_montant', [
			'trace' => $trace_id,
			'is_payant' => 'oui',
			'montant_total_calc_tr' => floatval($calc_tr['montant_total'] ?? 0),
			'montant_total_data_form' => floatval($data_form['montant_total'] ?? 0),
			'montant_total_retenu' => floatval($montant_total),
			'transaction_calc_count' => is_array($calc_tr['transaction'] ?? null) ? count($calc_tr['transaction']) : 0,
		...
		if (association_evenements_paiements_actifs()) {
			...
			$transaction_persistante = $id_transaction > 0 ? association_evenements_transaction_lire($id_transaction) : [];
			...
			association_evenements_inscription_debug_message('[IE_TRAITER_TRANSACTION][' . $trace_id . '] ' . json_encode([
	...
	association_evenements_inscription_debug_message('[IE_TRAITER_PERSIST][' . $trace_id . '] ' . json_encode([
	...
	$flux_traiter_post = pipeline('association_inscription_evenement_traiter', [
		'args' => [
			'etape' => 'apres',
			'mode' => $mode,
			'id_evenement' => intval($id_evenement),
...
function ie_resoudre_redirection_inscription($mode, $id_evenement, $id_activite_result, $cal_result, $affichage, $is_payant, $id_transaction) {
	...
	if ($rediriger_vers_paiement && !empty($id_transaction)) {
		$querie_transaction = association_evenements_transaction_lire($id_transaction);
...
function ie_format_post($mode, $id_evenement = 0, $post = null) {
	$post = ie_requete_attendue($mode, $id_evenement, intval(_request('id_activite')) ?: null, is_array($post) ? $post : []);
...
function ie_calculer_transaction($id_evenement, $data_form) {
	...
	if (!empty($data_form['montant_total']) || isset($data_form['transaction'])) {
		association_evenements_inscription_debug('calcul_transaction_fastpath', [
			'trace' => $trace_id,
			'id_evenement' => intval($id_evenement),
			'montant_total_data_form' => floatval($data_form['montant_total'] ?? 0),
			'transaction_count' => is_array($data_form['transaction'] ?? null) ? count($data_form['transaction']) : 0,
		...
		return [
			'montant_total' => $data_form['montant_total'] ?? 0,
			'transaction' => $data_form['transaction'] ?? [],
		];
	}

	$categorie_result = $data_form['categorie_result'] ?? ($data_form['categorie'] ?? false);
	$nombre_participants = $data_form['nombre_participants'] ?? 0;
	association_evenements_inscription_debug('calcul_transaction_fallback', [
		'trace' => $trace_id,
		'id_evenement' => intval($id_evenement),
		'categorie_count' => is_array($categorie_result) ? count($categorie_result) : 0,
	...
	return [
		'montant_total' => $res['montant_total'] ?? 0,
...
function ie_handle_transaction($montant_total, $id_auteur, $id_transaction = null) {
	...
	if ($id_transaction < 1) {
		...
		association_evenements_inscription_debug_message('[IE_HANDLE_TR_INSERT][' . $trace_id . '] ' . json_encode([
			'montant_total' => floatval($montant_total),
			'id_auteur' => intval($id_auteur),
			'id_transaction' => intval($id_new),
		]), 'association' . _LOG_DEBUG);
		return ['id_transaction' => $id_new, 'created' => true];
	} else {
		// modifier
		modifier_transaction_activites($montant_total, $id_transaction);
		association_evenements_inscription_debug_message('[IE_HANDLE_TR_UPDATE][' . $trace_id . '] ' . json_encode([
			'montant_total' => floatval($montant_total),
			'id_auteur' => intval($id_auteur),
			'id_transaction' => intval($id_transaction),
		]), 'association' . _LOG_DEBUG);
		return ['id_transaction' => $id_transaction, 'created' => false];
	}
}
 ...
 * Actuellement no-op.
 */
function ie_postprocess($id_activite, $data_form, $conf = []) {
	return true;
}
...
function ie_generer_saisies_par_etapes($saisies) {
	...
	foreach ((array) $saisies as $saisie) {
		...
		$positions_options = $saisie['options'] ?? [];
		$nom = $positions_options['nom'] ?? 'etape_' . ++$position;
		$label = $positions_options['label'] ?? ucfirst(str_replace('_', ' ', $nom));
		$etapes[] = [
			'nom' => $nom,
			'label' => $label,
		];
 ...
 */
function ie_assembler_saisies_multi($opts, $fieldsets = [], $hidden = []) {
	$saisies = [];
 ...
 */
function ie_requete_attendue($mode, $id_evenement = 0, $id_activite = null, $seed = []) {
	$res = is_array($seed) ? $seed : [];
	...
	if (strpos($mode, 'multi') !== false) {
		$nb_inscrits = intval(_request('nb_inscrits'));
		$post_etapes_precedentes = $res['cvtm_prev_post'] ?? [];
		if (!is_array($post_etapes_precedentes) && $post_etapes_precedentes !== '') {
			$post_cvt_normalise = ie_convertir_post(['cvtm_prev_post' => $post_etapes_precedentes]);
			$post_etapes_precedentes = $post_cvt_normalise['cvtm_prev_post'] ?? [];
		}
		...
		if ($nb_inscrits_a_collecter <= 0) {
			foreach (['prenom_inscrit_1', 'nom_inscrit_1', 'email_inscrit_1', 'telephone_inscrit_1', 'categorie_inscrit_1'] as $champ_premier_inscrit) {
		...
		$membres_famille = ['adherent', 'conjoint', 'enfant_1', 'enfant_2', 'enfant_3', 'enfant_4', 'enfant_5', 'invite_1', 'invite_2'];
