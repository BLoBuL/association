...
function champs_saisies_selection_membres_famille($data_famille, $accompagnants, $options_invites = array()) {
    $saisies = array();
...
function champs_saisies_inscrits($affichage_dans_activites,$nb_inscrits=1,$info_auteur_connecte=array()){
    $defaut = array();
    $query_auteur = null;
    $array_type_adherents = array();

    // Initialiser les défauts vides pour chaque inscrit possible
    for ($j = 1; $j <= 7; $j++) {
        $defaut["inscrit_$j"] = array('prenom' => '', 'nom' => '', 'email' => '', 'telephone' => '', 'date_naissance' => '');
    }
    ...
    if(isset($info_auteur_connecte['id_auteur']) && $id_auteur = $info_auteur_connecte['id_auteur']){
        ...
        if ($query_auteur) {
            $defaut['inscrit_1'] = array('prenom' => $query_auteur['prenom'] ?? '', 'nom' => $query_auteur['nom_famille'] ?? '', 'email' => $query_auteur['email'] ?? '', 'telephone' => $query_auteur['mobile'] ?? '');
            $defaut['inscrit_2'] = array('prenom' => $query_auteur['prenom_conjoint'] ?? '', 'nom' => $query_auteur['nom_conjoint'] ?? '', 'email' => $query_auteur['email_conjoint'] ?? '', 'telephone' => $query_auteur['mobile_conjoint'] ?? '');
            // Fix E : pré-remplir le nom de famille (partagé) pour les enfants
            $nom_famille_defaut = $query_auteur['nom_famille'] ?? '';
            $defaut['inscrit_3'] = array('prenom' => $query_auteur['prenom_enfant_1'] ?? '', 'nom' => $nom_famille_defaut, 'date_naissance' => $query_auteur['date_naissance_enfant_1'] ?? '');
            $defaut['inscrit_4'] = array('prenom' => $query_auteur['prenom_enfant_2'] ?? '', 'nom' => $nom_famille_defaut, 'date_naissance' => $query_auteur['date_naissance_enfant_2'] ?? '');
            $defaut['inscrit_5'] = array('prenom' => $query_auteur['prenom_enfant_3'] ?? '', 'nom' => $nom_famille_defaut, 'date_naissance' => $query_auteur['date_naissance_enfant_3'] ?? '');
            $defaut['inscrit_6'] = array('prenom' => $query_auteur['prenom_enfant_4'] ?? '', 'nom' => $nom_famille_defaut, 'date_naissance' => $query_auteur['date_naissance_enfant_4'] ?? '');
            $defaut['inscrit_7'] = array('prenom' => $query_auteur['prenom_enfant_5'] ?? '', 'nom' => $nom_famille_defaut, 'date_naissance' => $query_auteur['date_naissance_enfant_5'] ?? '');
        }
    }
    ...
    if ($affichage_dans_activites['payant'] == true) {
        if ($query_auteur) {
            if (($query_auteur['statut_interne'] ?? '') == 'ok' && in_array($query_auteur['statut'] ?? '', array('1comite','0minirezo'))) {
...
for ($i = 1; $i <= $nb_inscrits; $i++) {
    ...
    $saisies_total[$id_inscrit]= array(
        'saisie' => 'fieldset',
        'options' => array(
...
function champs_saisies_famille($id_auteur,$affichage_dans_activites){
    ...
    $saisies_total= array();
    if($query_auteur['statut_interne'] == 'ok' AND in_array($query_auteur['statut'], array('1comite','0minirezo'))){
            $array_type_adherents = array('adherent','indifferent','benevole');
    }elseif($query_auteur['statut_interne'] == 'ok'){
        $array_type_adherents = array('adherent','indifferent');
    }else{
    ...
    $famille['adherent'] = array('prenom' => $query_auteur['prenom'],'nom' => $query_auteur['nom_famille'],'email' => $query_auteur['email'],'telephone' => $query_auteur['mobile']);
    $famille['conjoint'] = array('prenom' => $query_auteur['prenom_conjoint'],'nom' => $query_auteur['nom_conjoint'],'email' => $query_auteur['email_conjoint'],'telephone' => $query_auteur['mobile_conjoint']);
    $famille['enfant_1'] = array('prenom' => $query_auteur['prenom_enfant_1'],'date_naissance' => $query_auteur['date_naissance_enfant_1']);
    $famille['enfant_2'] = array('prenom' => $query_auteur['prenom_enfant_2'],'date_naissance' => $query_auteur['date_naissance_enfant_2']);
    $famille['enfant_3'] = array('prenom' => $query_auteur['prenom_enfant_3'] ?? '','date_naissance' => $query_auteur['date_naissance_enfant_3'] ?? '');
    $famille['enfant_4'] = array('prenom' => $query_auteur['prenom_enfant_4'] ?? '','date_naissance' => $query_auteur['date_naissance_enfant_4'] ?? '');
    $famille['enfant_5'] = array('prenom' => $query_auteur['prenom_enfant_5'] ?? '','date_naissance' => $query_auteur['date_naissance_enfant_5'] ?? '');
    $famille['invite_1'] = array('prenom' => $query_auteur['invite_1'] ?? '');
    $famille['invite_2'] = array('prenom' => $query_auteur['invite_2'] ?? '');

    if(!empty(_request('famille'))){
    $request_famille = is_array(_request('famille')) ? array_flip(_request('famille')) : array(_request('famille')=> 0);
...
function champs_saisies_tarifs($id_inscrit,$premier_inscrit,$array_type_adherents,$tableau_categories,$compteur,$has_conjoint=false){
    $saisies_tarifs=$tarif_de_groupe= $afficher_si=$saisies_total= array();
    ...
    $disable = false;
      ...
      $saisies_tarifs[$id_inscrit]['saisies'][]= array(
        'saisie' => 'radio',
        'options' => array(
...
function champs_saisies_invites($nb_invite, $affichage_dans_activites) {
    ...
    $saisies_raw = champs_saisies_inscrits($affichage_dans_activites, $nb_invite, array());
...
function champs_saisies_info_supplementaire($id_inscrit,$info_supplementaire,$compteur=0,$defaut_values=array()){
    include_spip('inc/saisies');
    $saisies = array();

    // Initialiser les variables
    $premier_inscrit = false;
    $array_info_supplementaire = array();
    $cas_autres_id = '';

    if($compteur==1) {
        $premier_inscrit = true;
    }
    $array_cas= array('email','document_identite','telephone','date_naissance','nationalite','fonction','entreprise');
