...
function formulaires_editer_evenement_charger_dist($id_evenement = 'new', $id_article = 0, $retour = '', $lier_trad = 0, $config_fonc = 'evenements_edit_config', $row = [], $hidden = '') {
	...
	$valeurs['parents_id'] = ['article|' . $valeurs['id_parent']];
	...
	[$valeurs['date_debut'], $valeurs['heure_debut']] = explode(' ', date('d/m/Y H:i', strtotime($valeurs['date_debut'])));
	[$valeurs['date_fin'], $valeurs['heure_fin']] = explode(' ', date('d/m/Y H:i', strtotime($valeurs['date_fin'])));
	...
	$valeurs['_affiche_inscription'] = $GLOBALS['agenda_affiche_inscription'] ?? false;
 ...
 */
function formulaires_editer_evenement_identifier_dist($id_evenement = 'new', $id_article = 0, $retour = '', $lier_trad = 0, $config_fonc = 'evenements_edit_config', $row = [], $hidden = '') {
	return serialize([intval($id_evenement), $lier_trad]);
}

function formulaires_editer_evenement_verifier_dist($id_evenement = 'new', $id_article = 0, $retour = '', $lier_trad = 0, $config_fonc = 'evenements_edit_config', $row = [], $hidden = '') {
	$erreurs = formulaires_editer_objet_verifier('evenement', $id_evenement, ['titre', 'date_debut', 'date_fin']);
...
function formulaires_editer_evenement_verifier_modifie_evenements_lies($id_evenement, $id_article) {
	...
	foreach ($valeurs as $k => $v) {
		if (!in_array($k, ['repetitions'])
			and strpos($k, '_') !== 0
			and null !== ($p = _request($k))
			and $p != $v) {
...
function formulaires_editer_evenement_traiter_dist($id_evenement = 'new', $id_article = 0, $retour = '', $lier_trad = 0, $config_fonc = 'evenements_edit_config', $row = [], $hidden = '') {
	...
	if (!intval($id_evenement)
		...
		evenement_modifier($res['id_evenement'], ['statut' => 'publie']);
	}
	// a la creation, documenter la date de creation
	if (!intval($id_evenement)) {
		evenement_modifier($res['id_evenement'], ['date_creation' => date('Y-m-d H:i:s')]);
	}
