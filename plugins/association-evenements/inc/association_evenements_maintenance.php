...
function asso_supprimer_transactions_inscriptions(array $inscriptions, $dry_run = true) {
	...
	if (!$ids_tx) {
		return ['supprimes' => 0, 'ids_activite_supprimables' => array_column($inscriptions, 'id_activite')];
	}
	...
	$protegees = array_flip($resultat['protegees'] ?? []);
...
function asso_supprimer_participations_evenements_orphelines($dry_run = true, $lot = 1000) {
	...
	foreach ($candidats as $row) {
		...
		if ($id_transaction && (($transactions[$id_transaction]['statut'] ?? '') === 'ok')) {
			continue;
		}
