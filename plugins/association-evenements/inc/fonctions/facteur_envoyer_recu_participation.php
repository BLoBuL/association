...
function facteur_envoyer_recu_participation($email_inscrit, $id_transaction, $id_activite, $type_recu, $raison = null) {
	...
	if ($type_recu == 'encaissement') {
		...
		if (!empty($email_inscrit)) {
			...
			$want = ['prenom', 'nom', 'nom_famille', 'nom_entreprise'];
			$have = [];
			if (is_array($desc) && isset($desc['field']) && is_array($desc['field'])) {
				foreach ($want as $w) {
					if (array_key_exists($w, $desc['field'])) {
						$have[] = $w;
					}
				}
			}
		...
		$sujet = _T(
			'association_evenements:email_recu_encaissement_participation_sujet',
			[
		...
		$sujet = _T(
			'association_evenements:email_recu_remboursement_participation_sujet',
			[
				'nom_adherent' => $nom_adherent,
				'titre_evenement' => $titre_evenement,
				'numero_recu' => $numero_recu]
	...
	$envoyer = facteur_envoyer_app($email_inscrit, $sujet, $html, $bcc, ['enqueued' => true, 'use_queue' => false]);
