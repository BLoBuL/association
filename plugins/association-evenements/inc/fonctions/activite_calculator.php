	...
	if (!empty($id_auteur)) {
		if (!$gestion['payant']) {
			...
			if ($query_activite) {
				...
				$query_activite = $transaction ? array_merge($query_activite, $transaction) : [];
			}
		}
	}
		...
		if (!$gestion['places']) {
			...
			if (!empty($id_auteur)) {
				...
				if ($contexte['statut_adherent'] == 'preinscrit') { // Cas modification préinscrit
					$limite_place_modification = ($nombre_inscrit_adherent >= $gestion_places['places_disponibles']) ? $query_activite['nombre_inscrits'] : $place_disponible_adherent;
				} // Juste les places de l'adhérent ou les places_dispo - l'adhérent
				else { // Cas modification inscrit
					$limite_place_modification = $query_activite['nombre_inscrits'] + $gestion_places['places_disponibles'];
				} // Places prises par l'adhérent ID + places dispo - l'adhérent
			} else { // Cas nouvelle inscription
				$limite_place_modification = $place_disponible_adherent;
			} // Places prises par l'adhérent ID + places dispo - l'adhérent
			...
			if (!empty($id_auteur) and $query_activite['statut'] == 'liste_attente') {
				...
				$places_total = (($gestion_places['places_disponibles'] > $gestion_places['places_en_attentes_disponible'])) ? $gestion_places['places_disponibles'] + $limite_place_modification : $gestion_places['places_en_attentes_disponible'];
			} // On récupère le plus grand des deux (pour ne pas dépasser la limite)
