 ...
 */
function facteur_envoyer_mail_activites($id_evenement = '', $type = '', $id_activite = []) {
...
function facteur_envoyer_mail_activite_adherent($id_activite, $id_evenement, $type, $evenement, $activite_responsable_array) {
	...
	[$sujet_email_adherent, $model_email_adherent] = _determiner_modeles_emails_adherent($type);
	...
	try {
		...
		$sujet = _T($sujet_email_adherent, [
			'evenement' => $nom_evenement,
			'datev' => $date_ev,
		]);

		$html = recuperer_fond($model_email_adherent, [
		...
		if (!facteur_envoyer_app($email_inscrit, $sujet, $html, '', ['enqueued' => true, 'use_queue' => false])) {
...
function facteur_envoyer_mail_activite_responsable($infos_activite, $type, $evenement, $activite_responsable_array) {
	...
	[$sujet_email_responsable, $model_email_responsable] = _determiner_modeles_emails_responsable($type, $nombre_inscrits);
	...
	try {
		...
		$sujet = _T($sujet_email_responsable, [
			'evenement' => $nom_evenement,
		...
		if (!empty($emails_responsables)) {
			...
			if (!facteur_envoyer_app($dest, $sujet, $html, $bcc, ['enqueued' => true, 'use_queue' => false])) {
