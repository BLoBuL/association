...
function action_gerer_activites_dist() {
	...
	if (!$id_evenement || !autoriser('gererinscriptions', 'evenement', $id_evenement)) {
		include_spip('inc/minipres');
		echo minipres();
		exit;
	}
	$id_activites = array_values(array_unique(array_filter(array_map('intval', (array) _request('selecteur_id_activite')))));
	$id_activites = array_column(sql_allfetsel('id_activite', 'spip_asso_activites', ['id_evenement=' . $id_evenement, sql_in('id_activite', $id_activites)]), 'id_activite');
	if (!$id_activites) {
		return;
	}
	...
	if ($_POST['action_activites'] == 'valider_activites') {
		...
		foreach ($id_activites as $id_activite) {
			...
			if ($notify_the_members == 1) {
				job_queue_add('facteur_envoyer_mail_activites', 'Notification - valider_activites', $arguments = [$id_evenement, $type, $id_activite_selection], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
			}
	...
	if ($_POST['action_activites'] == 'devalider_activites') {
		...
		foreach ($id_activites as $id_activite) {
			...
			$query_asso_activites = sql_fetsel('id_transaction,statut,journal', 'spip_asso_activites', "id_activite = $id_activite");
			if ($notify_the_members == 1) {
				job_queue_add('facteur_envoyer_mail_activites', 'Notification - devalider_activites', $arguments = [$id_evenement, $type, $id_activite_selection], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
			}
	...
	if ($_POST['action_activites'] == 'mettre_en_attente_activites') {
		...
		foreach ($id_activites as $id_activite) {
			...
			$query_asso_activites = sql_fetsel('id_transaction,statut,journal', 'spip_asso_activites', "id_activite = $id_activite");
			if ($notify_the_members == 1) {
				job_queue_add('facteur_envoyer_mail_activites', 'Notification - mettre_attente_activites', $arguments = [$id_evenement, $type, $id_activite_selection], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
			}
	...
	if ($_POST['action_activites'] == 'desinscrire_activites') {
		...
		foreach ($id_activites as $id_activite) {
			...
			$query_transaction = $id_transaction > 0 ? association_evenements_transaction_lire($id_transaction) : [];

			if ($notify_the_members == 1) {
				job_queue_add('facteur_envoyer_mail_activites', 'Notification - desinscription_backend', $arguments = [$id_evenement, $type, $id_activite_selection], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
			}
			...
			sql_updateq('spip_asso_activites', ['statut' => 'desinscrit', 'journal' => $entree_journal], "id_activite=$id_activite");
			if ($id_transaction > 0 && isset($query_transaction['statut']) && $query_transaction['statut'] != 'ok') {
				association_evenements_transaction_modifier($id_transaction, ['statut' => 'abandon']);
			}
	...
	if ($_POST['action_activites'] == 'reactiver_activites') {
		...
		foreach ($id_activites as $id_activite) {
			...
			if (
				...
				&& in_array($query_transaction['statut'], ['abandon', 'echec'])
			) {
				association_evenements_transaction_modifier($id_transaction, ['statut' => 'attente']);
			}

			if ($notify_the_members == 1) {
				job_queue_add('facteur_envoyer_mail_activites', 'Notification - reactiver_activites', $arguments = [$id_evenement, $type, $id_activite_selection], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
			}
		}
	}
