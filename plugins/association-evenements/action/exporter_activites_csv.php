...
function action_exporter_activites_csv_dist() {
	...
	if ($config_evenement['payant']) {
		...
		$colonnes_paiement = ['montant', 'paiement_statut'];
	}
	...
	$remplacement = ['a.', 'b.', 'c.', 'statut AS ', 'statut AS '];
	...
	if (sql_count($query_activites)) {
		...
		foreach ($lignes as $adherent_array) {
			...
			if ($config_evenement['payant']) {
				$transaction = $transactions[(int) ($adherent_array['id_transaction'] ?? 0)] ?? [];
		...
		$titre = _T('association_evenements:titre_csv_activites', ['id_evenement' => $id_evenement]) . '-' . $GLOBALS['meta']['nom_site'] . '-' . date('Y-m-d');
