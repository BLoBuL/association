...
function action_ajouter_activites() {
	return action_ajouter_activites_dist();
}

function action_ajouter_activites_dist() {
	...
	if ($id_evenement <= 0 || $id_auteur <= 0 || !autoriser('gererinscriptions', 'evenement', $id_evenement)) {
		return false;
	}
	$id_activite = (int) $id_activite;
	if ($id_activite && (int) sql_getfetsel('id_evenement', 'spip_asso_activites', 'id_activite=' . $id_activite) !== $id_evenement) {
		return false;
	}
...
function activites_insert($categorie_result, $date, $id_evenement, $id_auteur, $nom_participants, $commentaire, $valider, $gratuit_single, $total_inscrits, $id_activite, $notify_the_members) {
	...
	if ($gratuit_single) {
		...
		if (!$cal_result['gestion']['validation'] or $valider) {
			...
			association_evenements_transaction_modifier($id_transaction, [
				'reglee' => 'oui',
				'statut' => 'ok',
				'finie' => 1,
				'montant_regle' => '0',
	...
	if ($notify_the_members == 1) {
		...
		job_queue_add('facteur_envoyer_mail_activites', 'Notification -ajout_activites', $arguments = [$id_evenement, $id_auteur, $type, $id_activite], $file = '', $no_duplicate = false, $time = 0, $priority = 0);
	}
}
