...
function action_synchroniser_comptabilite_evenement_dist() {
	...
	if (!association_plugin_actif('association_compta')
		|| !autoriser('gererinscriptions', 'evenement', $id_evenement)) {
		return false;
	}
...
function synchroniser_comptabilite_evenement($id_evenement) {
	...
	foreach ($activites as $activite) {
		...
		$comptes_transaction = association_compta_ecritures_objet_lister('evenement', $id_evenement, [
		...
		if ($compte_paiement && !isset($transactions_amount_synced[$id_transaction])) {
			...
			if (abs($compte_recette - $transaction_montant) > 0.001) {
				...
				association_compta_ecriture_modifier((int) $compte_paiement['id_compte'], ['recette' => $transaction_montant]);
		...
		if (!$compte_paiement) {
			...
			if ($id_paiement) {
				$compte_paiement = ['id_compte' => $id_paiement, 'vu' => 0, 'date' => $date_inscription ?: ''];
				$ajoutes++;
			}
		}
		...
		if ($compte_paiement) {
			if ($est_encaissee && !$compte_paiement['vu']) {
				association_compta_ecriture_modifier((int) $compte_paiement['id_compte'], ['vu' => 1]);
				$compte_paiement['vu'] = 1;
				$maj++;
			}
			if ($date_inscription && $compte_paiement['date'] !== $date_inscription) {
				association_compta_ecriture_modifier((int) $compte_paiement['id_compte'], ['date' => $date_inscription]);
	...
	$comptes = association_compta_ecritures_objet_lister('evenement', $id_evenement, ['champs' => 'id_compte,id_transaction']);
...
function nettoyer_doublons_comptabilite($id_evenement) {
	...
	$comptes_evenement = association_compta_ecritures_objet_lister('evenement', $id_evenement, [
