...
function synchroniser_comptabilite_evenement($id_evenement) {
    ...
    if ($payant !== null && intval($payant) === 0) {
        $nb = sql_delete('spip_asso_comptes', "objet='evenement' AND id_objet=" . intval($id_evenement) . " AND id_transaction>0");
        $supprimes += intval($nb);
        return array(
            'status' => 'success',
            'ajoutes' => 0,
    ...
    $activites = sql_allfetsel(
        'a.id_activite, a.id_transaction, a.statut as statut_activite, a.`date` as date_inscription, t.statut as statut_transaction, t.reglee as transaction_reglee, t.montant as transaction_montant',
        'spip_asso_activites a LEFT JOIN spip_transactions t ON a.id_transaction=t.id_transaction',
        "a.id_evenement=" . intval($id_evenement) . " AND a.id_transaction > 0"
    );
    ...
    foreach ($activites as $activite) {
        ...
        $statut_transaction = $activite['statut_transaction'];
        $date_inscription = !empty($activite['date_inscription']) ? $activite['date_inscription'] : null;
        $transaction_montant = isset($activite['transaction_montant']) ? floatval($activite['transaction_montant']) : 0.0;
        ...
        $comptes_transaction = sql_allfetsel(
            'id_compte, vu, `date`, recette, depense',
            'spip_asso_comptes',
            "id_transaction=" . intval($id_transaction)
            . " AND objet='evenement'"
            . ' AND id_objet=' . intval($id_evenement)
        );
        ...
        if ($compte_paiement && !isset($transactions_amount_synced[$id_transaction])) {
            ...
            if (abs($compte_recette - $transaction_montant) > 0.001) {
                ...
                sql_updateq('spip_asso_comptes', array('recette' => $transaction_montant), 'id_compte=' . intval($compte_paiement['id_compte']));
                ...
                // Recharger le compte_paiement pour refléter la mise à jour
                $compte_paiement = sql_fetsel('id_compte, vu, `date`, recette, depense', 'spip_asso_comptes', 'id_compte=' . intval($compte_paiement['id_compte']));
            } else {
                $transactions_amount_synced[$id_transaction] = true;
            }
        }
        ...
        if ($compte_paiement) {
            if ($est_encaissee && !$compte_paiement['vu']) {
                sql_updateq('spip_asso_comptes', array('vu' => 1), 'id_compte=' . intval($compte_paiement['id_compte']));
                $maj++;
            }
            if ($date_inscription && $compte_paiement['date'] !== $date_inscription) {
                sql_updateq('spip_asso_comptes', array('date' => $date_inscription), 'id_compte=' . intval($compte_paiement['id_compte']));
                $maj++;
            }
        }
        ...
        if ($est_rembourse) {
            ...
            foreach ($ids_remboursement as $id_compte_remboursement) {
                sql_delete('spip_asso_comptes', 'id_compte=' . intval($id_compte_remboursement));
                $supprimes++;
            }
        }
    }
    ...
    $comptes = sql_allfetsel(
        'id_compte, id_transaction',
        'spip_asso_comptes',
        "id_objet=" . intval($id_evenement) . " AND id_transaction > 0 AND objet='evenement'"
    );
    ...
    foreach ($comptes as $compte) {
        ...
        if (!in_array($compte['id_transaction'], $ids_transactions_activites)) {
            sql_delete('spip_asso_comptes', "id_compte=" . intval($compte['id_compte']));
            $supprimes++;
        }
    }
...
function nettoyer_doublons_comptabilite($id_evenement) {
    ...
    // 1. Trouver les transactions qui ont plusieurs entrées comptables
    $doublons = sql_allfetsel(
        'id_transaction, COUNT(*) as nb_comptes',
        'spip_asso_comptes',
        "id_transaction > 0 AND objet='evenement' AND id_objet=" . intval($id_evenement),
        'id_transaction', // GROUP BY
        '', // ORDER BY (chaîne vide, pas null)
        '', // LIMIT (chaîne vide)
        'nb_comptes > 1' // HAVING (sans le mot-clé "HAVING")
    );

    // 2. Pour chaque transaction avec doublons
    foreach ($doublons as $doublon) {
        $id_transaction = $doublon['id_transaction'];

        // Récupérer tous les comptes pour cette transaction, triés par "vu" puis par id
        $comptes = sql_allfetsel(
            'id_compte, vu, recette, depense',
            'spip_asso_comptes',
            "id_transaction=" . intval($id_transaction)
            . " AND objet='evenement'"
            . ' AND id_objet=' . intval($id_evenement),
            '', // Pas de GROUP BY
            'vu DESC, id_compte ASC', // Priorité aux comptes marqués comme vus
            '' // Pas de LIMIT
        );
