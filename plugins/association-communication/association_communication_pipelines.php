...
function association_communication_association_config_cli_registre($flux) {
	include_spip('inc/association_communication_config_cli');
	$flux['data'] = association_config_cli_ajouter_definitions($flux['data'], association_communication_config_cli_definitions());
	return $flux;
}

function association_communication_association_programmer_campagne($campagne) {
	include_spip('inc/email_collectif');
	$campagne['id_mailshot'] = association_communication_mailshot_creer(
		(string) $campagne['sujet'],
		(string) $campagne['html'],
		(array) $campagne['destinataires'],
		(array) $campagne['options']
	);
	return $campagne;
}

function association_communication_association_capacites($capacites) {
	$capacites['communication'] = ['plugin' => 'association_communication'];
	$capacites['notifications_metier'] = ['plugin' => 'association_communication'];
	$capacites['campagnes_email'] = ['plugin' => 'association_communication'];
...
function association_communication_association_notifier_metier($notification) {
	...
	if ($fonction && is_callable($fonction)) {
		$arguments = (array) ($notification['arguments'] ?? []);
...
function association_communication_association_configuration_saisies($flux) {
	...
	$flux['data'][] = ['ordre' => 10, 'saisies' => association_communication_configurer_saisies($flux['args']['config'] ?? '')];
	return $flux;
}
...
function association_communication_association_configuration_verifier($flux) {
	...
	$flux['data'][] = ['ordre' => 20, 'erreurs' => association_communication_configurer_verifier($flux['args']['config'] ?? '')];
	return $flux;
}
...
function association_mailsubscriber_formater_informations_liees($champs_extra, $liste_informations_segmentables) {
	...
	$opts = isset($champs_extra['options']) && is_array($champs_extra['options']) ? $champs_extra['options'] : [];
...
function association_mailsubscriber_informations_liees($flux) {
	...
	$raw_segment = $GLOBALS['association_metas']['selection_segment'] ?? null;
	...
	if (isset($flux['args']['declarer'])) {
		...
		$flux['data']['adherent'] = [
			...
			'options' => [
				'nom' => 'adherent',
				'label' => $t,
				'datas' => ['oui' => 'Oui', 'non' => 'Non'],
			],
		];
		$flux['data']['statut_interne'] = [
			'titre' => $t = 'Statut de l\'adhérent',
			'saisie' => 'selection',
			'options' => [
				'nom' => 'statut_interne',
				'label' => $t,
				'datas' => ['ok' => 'A jour', 'prospect' => 'Nouvel inscrit', 'echu' => 'Echus', 'relance' => 'Relancé', 'sorti' => 'Désactivé'],
			],
		];
		// Année d'inscription
		$flux['data']['inscription'] = [
			'titre' => $t = 'Année de première inscription',
			'saisie' => 'selection_multiple',
			'options' => [
				'nom' => 'inscription',
				'label' => $t,
				'datas' => ['2026' => '2026', '2025' => '2025', '2024' => '2024', '2023' => '2023', '2022' => '2022', '2021' => '2021', '2020' => '2020', '2019' => '2019', '2018' => '2018', '2017' => '2017', '2016' => '2016', '2015' => '2015', '2014' => '2014', '2013' => '2013'],
				'size' => '5',
			],
		];
		// Année de validité
	...
	if (isset($flux['args']['email']) && $flux['args']['email'] && !isset($flux['args']['declarer'])) {
		...
		$liste_segment_perso = ['statut_interne', 'inscription', 'validite'];
		...
		if (isset($query_auteur['quartier_rattachement']) or isset($query_auteur['quartier_coeur'])) {
			$flux['data']['quartiers'] = [$query_auteur['quartier_rattachement'] ?? '', $query_auteur['quartier_coeur'] ?? ''];
		}
	}
	return $flux;
}
...
function association_communication_notifications_destinataires($flux) {
	...
	if (
		...
		$flux['data'] = association_collecter_destinataires_admins() ?: [];
	}
	return $flux;
}
