...
function association_formulaire_email_collectif_fichiers() {
    return array('visuel_principal', 'documents_joints');
}
...
//ETAPE 1
    $saisies_fieldset_edition_message= array(
        'saisie' => 'fieldset',
        'options' => array(
            ...
            array(
                ...
                'verifier' => array(
                    ...
                    'options' => array(
                        'taille_max' => 12000,
                        'mime' 	=>'specifique',
                        'mime_specifique' => array('image/jpeg','image/png')
                    )
                )
            ),
            array(
...
function association_formulaire_email_collectif_charger($mode = 'adherent', $id_evenement = 0){
    ...
    if ($id_evenement) {
        ...
        if (!is_array($selection_activites)) {
            $selection_activites = $selection_activites ? array($selection_activites) : array();
        }
    ...
    if (!empty($selected_gabarit)){
        ...
        $contexte+= array_merge($contenu_gabarit,array('selected_gabarit'=>$selected_gabarit));
    }
    ...
    elseif(isset($_GET['_etape']) AND $_GET['_etape']==4 AND !$source_inscrits_evenement){
        ...
        if (empty($criteres_sql) && $recherche_adherents) {
            $liste_auteurs = generer_array_adherents(array("statut_interne = 'ok'"));
    ...
    if (!is_array($tmp_sel)) {
        $tmp_sel = $tmp_sel ? array($tmp_sel) : array();
    }
...
function association_formulaire_email_collectif_traiter($mode = 'adherent', $id_evenement_contexte = 0){
    ...
    $html= recuperer_fond("notifications/email_collectif",
        array(
            'titre' =>$titre,
            'chapeau' => $chapeau,
    ...
    if (!is_array($selecteur_adherent)) {
        $selecteur_adherent = $selecteur_adherent ? array($selecteur_adherent) : array();
    }
    ...
    if (!is_array($selecteur_activite_evenement)) {
        $selecteur_activite_evenement = $selecteur_activite_evenement ? array($selecteur_activite_evenement) : array();
    }
