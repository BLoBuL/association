...
function association_notifications_audits_html() {
	$flux = pipeline('association_notifications_audit_html', ['data' => '']);
...
function formulaires_editer_newsletter_traiter($id_newsletter = 'new', $retour = '', $lier_trad = 0, $config_fonc = 'newsletter_edit_config', $row = [], $hidden = '') {
	$baked = 1;
	$statut = (intval($id_newsletter) ? sql_getfetsel('statut', 'spip_newsletters', 'id_newsletter=' . intval($id_newsletter)) : 'prepa');
	if (in_array($statut, ['prepa', 'prop', 'prog'])) {
	...
	if (!$baked and $res['id_newsletter']) {
		...
		objet_dissocier(['newsletter' => $res['id_newsletter']], $possibles);
		if (count($liens)) {
			objet_associer(['newsletter' => $res['id_newsletter']], $liens);
		}
