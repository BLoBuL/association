...
function association_dons_association_configuration_saisies($flux) {
	include_spip('formulaires/inc/configurer_association_dons');
	$flux['data'][] = [
		'ordre' => 60,
		'saisies' => association_dons_configurer_saisies($flux['args']['config'] ?? ''),
	];
	return $flux;
}
...
function association_dons_association_capacites($capacites) {
	$capacites['dons'] = ['plugin' => 'association_dons'];
...
function association_dons_association_rgpd_anonymiser_auteur($flux) {
	...
	$flux['data']['dons_anonymises'] = association_rgpd_updateq('spip_asso_dons', association_rgpd_filtrer_champs('spip_asso_dons', [
