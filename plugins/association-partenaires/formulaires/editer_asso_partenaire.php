...
function formulaires_editer_asso_partenaire_charger_dist($id_partenaire = 'new', $retour = '') {
	$valeurs = formulaires_editer_objet_charger('asso_partenaire', $id_partenaire, 0, 0, $retour, '');
	$valeurs['_organisations'] = array();
...
function formulaires_editer_asso_partenaire_verifier_dist($id_partenaire = 'new', $retour = '') {
	$erreurs = formulaires_editer_objet_verifier('asso_partenaire', $id_partenaire, array('id_organisation'));
	if (intval(_request('id_organisation')) < 1) { $erreurs['id_organisation'] = _T('info_obligatoire'); }
	return $erreurs;
}

function formulaires_editer_asso_partenaire_traiter_dist($id_partenaire = 'new', $retour = '') {
	return formulaires_editer_objet_traiter('asso_partenaire', $id_partenaire, 0, 0, $retour, '');
}
