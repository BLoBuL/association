 ...
 */
function facteur_envoyer_app($destinataire, $sujet, $html, $bcc=false, $options=array()) {
    ...
    if (empty($emails)) {
        return array('success' => false, 'message' => 'Aucun destinataire valide');
    }

    if (empty($sujet)) {
        return array('success' => false, 'message' => 'Sujet manquant');
    }
    if (empty($html)) {
        return array('success' => false, 'message' => 'Contenu du message manquant');
    }
    ...
    if ($use_queue && empty($options['enqueued'])) {
            ...
            job_queue_add('facteur_envoyer_app', $titre, array($emails, $sujet, $html, $bcc, $opts), '', false, 0, 0);
            association_log('email', 'facteur_envoyer_app : job mis en queue pour ' . implode(', ', $emails) . ' sujet: ' . $sujet, 'info');
            return array('success' => true, 'message' => 'Email mis en file (job_queue)');
    ...
    try {
        ...
        if ($sent > 0) {
            return array('success' => true, 'message' => 'Email(s) envoyé(s) : ' . $sent . (count($emails) > 1 ? ' / ' . count($emails) : ''));
        }
        return array('success' => false, 'message' => 'Aucun envoi réussi : ' . implode(' ; ', $errors));
