...
function association_paiements_association_capacites($capacites) {
	$capacites['paiements'] = ['plugin' => 'association_paiements'];
	$capacites['transactions'] = ['plugin' => 'association_paiements'];
...
function association_paiements_association_configuration_saisies($flux) {
	...
	$flux['data'][] = ['ordre' => 40, 'saisies' => association_paiements_configurer_saisies(
		$flux['args']['config'] ?? '',
...
function association_trig_bank_notifier_reglement($flux) {
	...
	pipeline('association_paiements_reglement_traiter', [
		'args' => [
			'id_transaction' => intval($id_transaction),
			'transaction' => is_array($query_transaction) ? $query_transaction : [],
		],
...
function association_bank_redirige_apres_retour_transaction($flux) {
	...
	if ($id_transaction
		...
		$redirection = pipeline('association_paiements_redirection_transaction', [
			'args' => [
				'id_transaction' => (int) $id_transaction,
				'transaction' => is_array($flux['args']['row'] ?? null) ? $flux['args']['row'] : [],
			],
			'data' => '',
		]);
...
function association_paiements_association_compta_ecritures_devises($flux) {
	...
	foreach ($ecritures as $ecriture) {
		...
		if ($id_compte && preg_match('/^[A-Z]{3}$/', $devise)) {
...
function association_paiements_association_paiements_transactions_informations($flux) {
	$ids = (array) ($flux['args']['ids_transactions'] ?? []);
	if (!empty($flux['args']['id_transaction'])) {
		$ids[] = (int) $flux['args']['id_transaction'];
	}
	include_spip('inc/association_paiements_transactions');
	...
	if (!$flux['data'] && !empty($flux['args']['id_commande'])) {
		...
		if ($transaction) {
			$flux['data'][(int) $transaction['id_transaction']] = $transaction;
		}
	}
	return $flux;
}
...
function association_paiements_association_maintenance_auteurs_encaisses($flux) {
	...
	$references = pipeline('association_paiements_transactions_references', [
		'args' => ['ids_transactions' => array_keys($par_id)],
		'data' => [],
	]);
...
function association_paiements_association_rgpd_anonymiser_auteur($flux) {
	...
	$flux['data']['transactions_anonymisees'] = association_rgpd_updateq('spip_transactions', association_rgpd_filtrer_champs('spip_transactions', [
