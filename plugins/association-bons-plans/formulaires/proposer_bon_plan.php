...
function formulaires_proposer_bon_plan_charger_dist($id_rubrique = 0, $retour = '') {
	...
	if (empty($GLOBALS['visiteur_session']['id_auteur'])) {
		...
		$valeurs['message_erreur'] = _T('association_bons_plans:connexion_requise');
	}
	return $valeurs;
}
...
function formulaires_proposer_bon_plan_traiter_dist($id_rubrique = 0, $retour = '') {
	...
	if ($id) {
		...
		objet_modifier('bon_plan', $id, ['statut' => 'prop']);
		if ($id_auteur) {
			include_spip('action/editer_liens');
			objet_associer(['bon_plan' => $id], ['auteur' => $id_auteur]);
		}
		...
		association_notifier_metier([
			...
			'arguments' => [$id, lire_config('association/bons_plans_moderateurs', '')],
		]);
...
function association_bons_plans_notifier_proposition($id_bon_plan, $destinataires) {
	...
	$sujet = _T('association_bons_plans:notification_sujet', ['titre' => $titre]);
	$message = _T('association_bons_plans:notification_texte', [
		'titre' => $titre,
	...
	foreach ($emails as $email) {
		$succes = (bool) $envoyer_mail($email, $sujet, $message) && $succes;
	}
	return $succes;
}
