	...
	if (intval($id_compte)) {
		...
		$admin_saisies[] = [
			...
			'verifier' => [
				...
				'options' => [
					'mime' => 'specifique',
					'mime_specifique' => ['application/pdf', 'image/jpeg', 'image/png'],
					'taille_max' => 10240,
				],
			],
		];
...
function formulaires_editer_asso_cotisation_verifier_dist($id_compte = 'new') {
	...
	$champs_obligatoires = ['id_categorie', 'date_operation'];
	...
	if (!isset($erreurs['id_categorie'])) {
		include_spip('inc/api_cotisations');
		$fichiers = _request('_fichiers') ?: ($_FILES ?? []);
	...
	foreach (['date_operation', 'date_fin_validite'] as $champ_date) {
		...
		if ($id_categorie) {
			...
			if ($cat && isset($cat['validation'])) {
				...
				if ($validation === 'pre-paiement') {
					...
					$allowed = ['demande', 'attente'];
				} elseif ($validation === 'post-paiement' || $validation === 'auto') {
					$allowed = ['attente', 'ok'];
				} else {
					// fallback, autoriser les statuts standards
					$allowed = ['demande', 'attente', 'ok'];
				}
...
function formulaires_editer_asso_cotisation_traiter($id_compte = 'new') {
	...
	if (!empty($fichiers)) {
		...
		if ($doc) {
			$params['documents'] = ['document_justificatif' => $doc];
		}
	}
