...
function adherents_recherche_avancee_statut_adhesion_saisie() {
	...
	$liste_champs_table = ['statut_interne', 'inscription', 'validite'];
...
function adherents_recherche_avancee_formater_saisie(array $champs_extra, array $liste_champs_table, $type_recherche) {
	...
	if ($champs_extra['saisie'] == 'case') {
		...
		$champs_extra['options']['data'] = saisies_tableau2chaine(['on' => 'Oui', '' => 'Non']);
 ...
 */
function generer_array_adherents($criteres_sql, $periode_selectionnee = []) {
	...
	if ($desc_auteurs) {
		if (!empty($desc_auteurs['field']['type_adherent'])) {
			$col_type_adherent = 'type_adherent';
		} elseif (!empty($desc_auteurs['field']['radio_type_adherent'])) {
			$col_type_adherent = 'radio_type_adherent';
		}
	}

	$champs = ['id_auteur', 'validite', 'inscription', 'statut', 'statut_interne'];
	...
	$champs = array_merge($champs, ['nom_famille', 'prenom', 'auteur_compte_principal', 'email']);
	...
	while ($data = sql_fetch($query_adherents)) {
		...
		$id_auteurs[$id] = [
			...
			'statut_interne' => $data['statut_interne'] ?? null,
			'type_adherent' => $data['type_adherent'] ?? null,
			'nom_famille' => $data['nom_famille'] ?? null,
...
function afficher_resultat_recherche_avancee($contexte) {
	...
	if (!empty($criteres_sql)) {
		...
		$fallback_criteres = ['validite >= ' . sql_quote($now)];
		if (_request('var_mode') === 'debug') {
		...
		return is_array($id_auteurs) && count($id_auteurs) ? $id_auteurs : [];
	}
