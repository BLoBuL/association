...
function genie_association_taches_generales($tache) {
	...
	$select_fields = ['id_auteur', 'email', 'statut', 'statut_interne', 'validite'];
	...
	while ($auteur = sql_fetch($query_auteurs)) {
		...
		if ($statut_interne == 'ok') {
			if ($date_validite && $date_validite < $date_actuelle_ymd) {
				...
				sql_updateq('spip_auteurs', ['statut' => '6forum', 'statut_interne' => 'echu'], "id_auteur=$id_auteur");
				...
				if (!empty($GLOBALS['association_metas']['notification_adherent_echu']) && $GLOBALS['association_metas']['notification_adherent_echu'] != 'non') {
					job_queue_add('association_job_notifier_echeance', 'Notification - notification_echeances_adherent_echu', [$id_auteur, $email_adherent, 0, 'echeance_echu', $type_adherent], '', true, 0, 0);
				}
			}
			...
			if ($nb_jour_differences !== null && $nb_jour_differences > 0 && !empty($delais_notification) && in_array($nb_jour_differences, $delais_notification, true)) {
				...
				job_queue_add('association_job_notifier_echeance', 'Notification - notification_echeances_adherent', [$id_auteur, $email_adherent, $nb_jour_differences, 'echeance_preventive', $type_adherent], '', true, $date_heure_envoi_timestamp, 0);

			}
		} elseif (in_array($statut, ['0minirezo', '1comite'])) {
			// Passe les administrateurs et rédacteurs en visiteurs si leur adhésion est expirée
			sql_updateq('spip_auteurs', ['statut' => '6forum'], "id_auteur=$id_auteur");
		}
