...
function action_valider_justificatifs_cotisation_dist() {
	...
	[$id_compte, $mode] = array_pad(explode('-', (string) $arg, 2), 2, 'oui');
	...
	if ($resultat['ok'] && $mode === 'non') {
		...
		if ($compte) {
			...
			$transaction = association_adhesions_transaction_lire((int) ($compte['id_transaction'] ?? 0));
			notifier_cotisation_adherent($compte, $categorie ?: [], $transaction ?: [], 'justificatifs-a-revoir');
		}
	}
