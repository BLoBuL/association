...
function association_lire_config_liste($meta){
    $cfg = isset($GLOBALS['association_metas'][$meta]) ? $GLOBALS['association_metas'][$meta] : lire_config('association_metas/' . $meta);
...
function filtre_liste_type_adherent(){
    ...
    if (is_array($liste_champs_extra)) {
        foreach ($liste_champs_extra as $champs_extra) {
            if (isset($champs_extra['options']['nom']) && in_array($champs_extra['options']['nom'], array('radio_type_adherent','type_adherent'))) {
                $datas = isset($champs_extra['options']['datas']) ? $champs_extra['options']['datas'] : '';
    ...
    if ($desc) {
        $cols = array();
        if (!empty($desc['field']['type_adherent'])) $cols[] = 'type_adherent';
        if (!empty($desc['field']['radio_type_adherent'])) $cols[] = 'radio_type_adherent';
    ...
    if (function_exists('radio_type_adherent')) {
        ...
        if (is_array($r) && count($r) > 0) return $r;
    }
...
function filtre_has_type_adherent(){
    ...
    $desc = sql_showtable('spip_auteurs', true);
    if ($desc) {
        if (!empty($desc['field']['type_adherent'])) return array('type_adherent');
        if (!empty($desc['field']['radio_type_adherent'])) return array('radio_type_adherent');
    }
...
function preparer_liste_adherents($data = '') {
    ...
    if ($context->periode) association_log('adherents', 'Période: ' . $context->periode, 'debug');
    if ($context->statut_interne) association_log('adherents', 'Statut: ' . $context->statut_interne, 'debug');
    if ($context->recherche_type) association_log('adherents', 'Type recherche: ' . $context->recherche_type, 'debug');
    ...
    if ($data === 'array_auteur') {
        return is_array($id_auteurs) ? $id_auteurs : array();
    } elseif ($data === 'criteres_sql') {
        return $criteres_sql;
    } elseif ($data === 'context') {
...
function association_donnees_colonnes_adherent($id_auteur){
    ...
    $cache[$id_auteur] = is_array($row) ? $row : array();
...
function valeur_colonne_dynamique_adherent($id_auteur, $champ){
    ...
    return isset($row[$champ]) ? $row[$champ] : '';
}

function formatter_valeur_colonne_dynamique($valeur, $options = array()){
...
function filtre_filtres_dynamiques_actifs_dist($filtres_effectifs = array()){
...
function filtre_liste_filtres_reset_adherents_dist(){
    $filtres = array('periode','statut_interne','type_adherent','type_compte','type_cotisation');
    foreach (liste_filtres_dynamiques_adherents() as $nom => $def) {
        $filtres[] = $nom;
    }
...
function association_liste_champs_recherche_avancee_actifs(){
    ...
    if (!empty($_SESSION['adherents_recherche_avancee']) && is_array($_SESSION['adherents_recherche_avancee'])) {
        foreach ($_SESSION['adherents_recherche_avancee'] as $champ => $valeur) {
            ...
            if (in_array($champ, array('recherche', 'action', 'formulaire_action', 'formulaire_action_args', 'formulaire_action_sign'))) {
                continue;
            }
            $params[] = $champ;
        }
...
function filtre_get_recherche_rapide_active_dist(){
     $champs = array('_input_nom_famille','_input_prenom','_input_email','_input_mobile');
     $actifs = array();
...
function association_aplatir_datas_saisies($datas){
    // Si la fonction existe déjà, l'utiliser directement
    if (function_exists('saisies_aplatir_tableau')) {
        return saisies_aplatir_tableau($datas, true);
    }

    // Tenter de charger le fichier du plugin Saisies
    if (function_exists('include_spip')) {
        // include_spip est la manière standard dans SPIP
        include_spip('inc/saisies_data');
    } else {
        // Au cas où include_spip ne serait pas disponible (environnement atypique), tenter un include direct
        @include_once 'inc/saisies_data.php';
    }

    // Si la fonction est désormais disponible, l'utiliser
    if (function_exists('saisies_aplatir_tableau')) {
        return saisies_aplatir_tableau($datas, true);
    }

    // Si la fonction reste introuvable, retourner un tableau vide et logguer l'anomalie
    association_log('adherents', 'association_aplatir_datas_saisies: saisies_aplatir_tableau introuvable après tentative de chargement', 'info');
