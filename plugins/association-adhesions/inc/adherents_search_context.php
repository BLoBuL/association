...
class AdherentsSearchContext
{
	...
	public $statut_interne;

	public $type_adherent;

	public $type_compte;

	public $type_cotisation; // Nouveau filtre basé sur les catégories

	public $filtres_dynamiques = [];

	// Recherche rapide (POST)
	public $recherche_type; // 'rapide' ou 'avancee'
	...
	public $type_recherche; // 'statut_adhesion' ou 'multicritere'

	public $operateur_recherche = 'AND';

	public $criteres_avances = [];

	// Métadonnées
	public $periode_data; // Objet période complet
	...
	public static function fromRequest() {
		...
		$session_rapide = is_array($session_rapide) ? $session_rapide : [];
		$session_avancee = session_get('adherents_recherche_avancee');
		$session_avancee = is_array($session_avancee) ? $session_avancee : [];
		$session_filtres = session_get('adherents_filtres');
		$session_filtres = is_array($session_filtres) ? $session_filtres : [];
		...
		$champs_recherche = ['_input_nom_famille', '_input_prenom', '_input_email', '_input_mobile'];
		if (($champ_a_effacer = _request('clear_search_field')) !== null) {
			$cibles = ($champ_a_effacer === '__all__') ? $champs_recherche : [$champ_a_effacer];
			foreach ($cibles as $champ) {
				if (!in_array($champ, $champs_recherche, true)) {
					continue;
				}
				if (isset($_REQUEST[$champ])) {
					unset($_REQUEST[$champ]);
				}
				if (isset($_GET[$champ])) {
					unset($_GET[$champ]);
				}
		...
		$raw_periode = $_REQUEST['periode'] ?? null;
		$raw_statut = $_REQUEST['statut_interne'] ?? null;
		$raw_type = $_REQUEST['type_adherent'] ?? null;
		$raw_compte = $_REQUEST['type_compte'] ?? null;
		...
		if ($ctx->statut_interne === 'tout') {
			$ctx->statut_interne = null;
		}

		$ctx->type_adherent = _request('type_adherent');
		if ($ctx->type_adherent === 'tout') {
			$ctx->type_adherent = null;
		}
		...
		if ($ctx->type_cotisation === 'tout') {
			$ctx->type_cotisation = null;
		}

		// 1.b Persistance des filtres dans la session si présents dans la requête
		$raw_type_cotisation = $_REQUEST['type_cotisation'] ?? null;
		$filters = [
			'periode' => $raw_periode,
		...
		foreach (['periode', 'statut_interne', 'type_adherent', 'type_compte', 'type_cotisation'] as $k) {
			if (!array_key_exists($k, $_REQUEST) // pas de paramètre explicite
				...
				if (!empty($session_filtres[$k])) {
					...
					if (in_array($k, ['periode', 'statut_interne', 'type_adherent', 'type_cotisation'], true) && $val === 'tout') {
		...
		$dyn_defs = function_exists('liste_filtres_dynamiques_adherents') ? liste_filtres_dynamiques_adherents() : [];
		...
		if (_request('clear_search')) {
			if (isset($_REQUEST['_input_nom_famille'])) {
				unset($_REQUEST['_input_nom_famille']);
			}
			if (isset($_REQUEST['_input_prenom'])) {
				unset($_REQUEST['_input_prenom']);
			}
			if (isset($_REQUEST['_input_email'])) {
				unset($_REQUEST['_input_email']);
			}
			if (isset($_REQUEST['_input_mobile'])) {
				unset($_REQUEST['_input_mobile']);
			}
			if (isset($_GET['_input_nom_famille'])) {
				unset($_GET['_input_nom_famille']);
			}
			if (isset($_GET['_input_prenom'])) {
				unset($_GET['_input_prenom']);
			}
			if (isset($_GET['_input_email'])) {
				unset($_GET['_input_email']);
			}
			if (isset($_GET['_input_mobile'])) {
				unset($_GET['_input_mobile']);
			}
			if (isset($_POST['_input_nom_famille'])) {
				unset($_POST['_input_nom_famille']);
			}
			if (isset($_POST['_input_prenom'])) {
				unset($_POST['_input_prenom']);
			}
			if (isset($_POST['_input_email'])) {
				unset($_POST['_input_email']);
			}
			if (isset($_POST['_input_mobile'])) {
				unset($_POST['_input_mobile']);
			}
		}
		...
		if (!$recherche_rapide_active && empty($_POST['type_recherche'])) {
			...
			if (!empty($session_rapide)) {
				$ctx->recherche_type = 'rapide';
				$ctx->nom = $session_rapide['_input_nom_famille'] ?? null;
				$ctx->prenom = $session_rapide['_input_prenom'] ?? null;
				$ctx->email = $session_rapide['_input_email'] ?? null;
				$ctx->mobile = $session_rapide['_input_mobile'] ?? null;
			}
			// Restaurer recherche avancée depuis session
			elseif (!empty($session_avancee)) {
				$ctx->recherche_type = 'avancee';
				$ctx->type_recherche = $session_avancee['type_recherche'] ?? null;
				$ctx->operateur_recherche = $session_avancee['operateur_recherche'] ?? 'AND';
	 ...
	 */
	public function toSQL($liste_periodes = []) {
		$criteres = [];
	...
	private function buildPeriodeSQL($liste_periodes) {
		...
		$norm_param = strtolower(str_replace([' ', '/', '\\'], '-', $p_param));

		foreach ($liste_periodes as $periode) {
			if (!isset($periode['libelle'])) {
				continue;
			}
			...
			$norm_lib = strtolower(str_replace([' ', '/', '\\'], '-', $libelle));
		...
		$type_periode = $GLOBALS['association_metas']['validite'] ?? 'annuelle';
		...
		if ($type_periode === 'scolaire') {
			...
			$date_scolaire_nouvelle = $GLOBALS['association_metas']['date_scolaire_nouvelle'] ?? '30/09';
			[$jour_nouvelle, $mois_nouvelle] = explode('/', $date_scolaire_nouvelle);
	...
	public function toURLParams() {
		...
		if ($this->statut_interne) {
			$params['statut_interne'] = $this->statut_interne;
		}
		if ($this->type_adherent) {
			$params['type_adherent'] = $this->type_adherent;
		}
