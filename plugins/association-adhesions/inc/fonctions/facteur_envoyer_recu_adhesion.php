...
function facteur_envoyer_recu_adhesion($id_auteur, $id_transaction, $type_recu, $raison = '') {
	...
	if (empty($query_transaction)) {
		...
		return [
			'success' => false,
			'message' => "Transaction introuvable : $id_transaction",
		];
	}
	...
	if ($query_transaction['montant'] == 0) {
		...
		return [
			'success' => false,
			'message' => "Montant de la transaction nul : $id_transaction",
		];
	}
	...
	$fields = ['email', 'nom', 'nom_famille', 'prenom'];
	...
	if (empty($query_auteur)) {
		...
		return ['success' => false, 'message' => "Auteur introuvable : $id_auteur"];
	}
	...
	$query_compte = $id_compte ? association_cotisation_lire_par_compte($id_compte) : [];
	if (empty($query_compte)) {
		association_log('notifications', "Compte introuvable pour la transaction : $id_transaction", 'critique');
		return ['success' => false, 'message' => "Compte introuvable pour la transaction : $id_transaction"];
	}
	...
	if ($type_recu == 'encaissement') {
		...
		$sujet = _T(
			'association_adhesions:email_recu_encaissement_adhesion_sujet',
			[
	...
	$email_adherent = $query_auteur['email'] ?? '';
	...
	// Envoi de l'email
	try {
		$envoyer = facteur_envoyer_app($email_adherent, $sujet, $html, $bcc, ['enqueued' => true, 'use_queue' => false]);
