...
function generer_export_csv_adherents($criteres_sql = false) {
	...
	while ($row = sql_fetch($affichage_table)) {
		$a = unserialize($row['valeur'], ['allowed_classes' => false]);
		if (!is_array($a)) {
			continue;
		}
		foreach ($a as $key => $value) {
			if ($value == 'on') {
				$_table = '_table';
				$fieldset = 'fieldset_';
				$replace_string = ['_table', '_nocreation'];
	...
	$action_pdf = generer_action_auteur('exporter_adherents_pdf', 'export');
	return '<form method="post" action="' . attribut_html($action) . '"><div>'
		. $res
		. '<p class="boutons"><button type="submit" class="submit">' . _T('association_adhesions:bouton_generer_csv') . '</button> '
		. '<button type="submit" class="submit" formaction="' . attribut_html($action_pdf) . '">' . _T('association_adhesions:bouton_generer_pdf') . '</button></p>'
		. '</div></form>';
}
