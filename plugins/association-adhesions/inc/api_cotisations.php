...
function api_cotisations_saisies_communes($params) {
	...
	$id_compte = $params['id_compte'] ?? 'new';
	...
	$type_adherent = $params['type_adherent'] ?? '';
	$origine = $params['origine'] ?? 'prive';
...
function api_traiter_cotisation($params) {
	try {
		...
		if ($justification === '') {
			$justification = _T('association_adhesions:justification_encaissement_cotisation', [
		...
		$statut_cotisation = $params['statut_cotisation'] ?? false;
		...
		$fichiers_requete = function_exists('_request') ? _request('_fichiers') : [];
		$fichiers_justificatifs = $params['documents'] ?? ($fichiers_requete ?: ($_FILES ?? []));
 ...
 */
function cotisation_verifier_documents_justificatifs($id_categorie, $files = [], $id_compte = 'new') {
	...
	foreach (cotisation_normaliser_documents_justificatifs($files) as $fichier) {
		...
		if ($erreur === UPLOAD_ERR_NO_FILE || trim((string) ($fichier['name'] ?? '')) === '') {
			continue;
		}
		if ($erreur !== UPLOAD_ERR_OK || intval($fichier['size'] ?? 0) < 1) {
			return ['valide' => false, 'message' => _T('association_adhesions:erreur_justificatif_upload'), 'nouveaux_requis' => max(0, $minimum - $existants)];
		}
		if (intval($fichier['size']) > 10 * 1024 * 1024) {
			return ['valide' => false, 'message' => _T('association_adhesions:erreur_justificatif_taille'), 'nouveaux_requis' => max(0, $minimum - $existants)];
		}
		...
		if ($tmp !== '' && is_file($tmp) && function_exists('finfo_open')) {
			...
			if ($finfo) {
				...
				finfo_close($finfo);
				if ($mime_detecte) {
					$mime = strtolower($mime_detecte);
				}
			}
		}
		if (!in_array($extension, $extensions, true) || ($mime !== '' && !in_array($mime, $mimes, true))) {
			return ['valide' => false, 'message' => _T('association_adhesions:erreur_justificatif_format'), 'nouveaux_requis' => max(0, $minimum - $existants)];
		}
		$valides++;
	}
	...
	if ($valides < $nouveaux_requis) {
		return ['valide' => false, 'message' => _T('association_adhesions:erreur_justificatif_minimum', ['nb' => $minimum]), 'nouveaux_requis' => $nouveaux_requis];
	}
	return ['valide' => true, 'message' => '', 'nouveaux_requis' => $nouveaux_requis, 'valides' => $valides, 'existants' => $existants];
}
...
function preparer_liste_categories($id_auteur, $origine = 'prive', $inscription = 'inscription', $type_adherent = '') {
	...
	while ($row = sql_fetch($query_categories)) {
		...
		if (
			...
			if ($row['nombre_enfants'] > 0 && $row['nombre_enfants'] <= $query_auteur['nb_enfants'] || $row['nombre_enfants'] == 0) {
				$categories[$id_categorie] = $valeur;
			}
		}
...
function traiter_upload_justificatif($id_auteur, $id_compte, $files = null) {
	...
	$fichiers = $files ?: (_request('_fichiers') ?: ($_FILES ?? []));
