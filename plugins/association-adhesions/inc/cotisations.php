...
function changer_statut_cotisation($id_compte, $origine = '', $notifier = true) {
	...
	$query_transaction = association_adhesions_transaction_lire((int) ($query_cotisation['id_transaction'] ?? 0));
 ...
 */
function notifications_cotisation_trouver_sujet($type, $contexte = []) {
	// valeurs par défaut
	$type = (string) $type;
 ...
 */
function notifier_cotisation_envoyer($destinataires, $sujet, $template, $contexte = [], $bcc = false, $opts = []) {
	...
	try {
		...
		return ['success' => false, 'message' => 'Erreur rendu template'];
	}
 ...
 */
function notifier_cotisation_adherent($query_cotisation, $query_categories, $query_transaction, $type = '', $options = []) {
	$reinscription = $query_cotisation['reinscription'] ?? '';
	...
	$send_now = function ($dest, $sujet, $html, $bcc = false) {
		...
		$res = facteur_envoyer_app($dest, $sujet, $html, $bcc, ['use_queue' => false]);
		if (is_array($res)) {
	...
	if (!empty($options) && is_array($options)) {
		...
		$autorise = ['nb_jour_differences', 'email_override', 'use_queue', 'lang', 'validite', 'id_categorie', 'id_auteur', 'id_compte', 'montant', 'nom_adherent', 'nom_entreprise', 'statut'];
		foreach ($autorise as $k) {
			if (array_key_exists($k, $options) && (is_scalar($options[$k]) || $options[$k] === null)) {
	...
	$emails_adh = parser_emails_depuis_config($GLOBALS['association_metas']['config_destinataires_creation_cotisation_adh'] ?? '');
	...
	if (empty($sujet)) {
		association_log('cotisations', 'notifier_cotisation_adherent: envoi bloqué - sujet manquant pour type=' . $type . ' id_compte=' . intval($contexte_notification['id_compte'] ?? 0) . ' args=' . json_encode(['nom' => $contexte_notification['nom_cible'] ?? '', 'type' => $contexte_notification['type_cotisation'] ?? '']), 'critique');
	...
	if ($type == 'attente_paiement') {
		...
		if ($use_queue) {
			...
			job_queue_add('notifier_cotisation_envoyer', 'Notification - attente_paiement ' . intval($contexte_notification['id_compte'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, $bcc, []], '', false, 0, 0);
			...
			if (!$ok) {
				spip_log('Envoi direct attente_paiement échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
		...
		$bcc = !empty($emails_adh) ? (count($emails_adh) == 1 ? $emails_adh[0] : $emails_adh) : false;
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - validation_post-paiement ' . intval($contexte_notification['id_compte'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, $bcc, []], '', false, 0, 0);
			...
			$ok = $send_now($email_adherent, $sujet, $html, $bcc);
			if (!$ok) {
				spip_log('Envoi direct validation_post-paiement échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
		...
		$bcc = !empty($emails_adh) ? (count($emails_adh) == 1 ? $emails_adh[0] : $emails_adh) : false;
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - validation_pre-paiement ' . intval($contexte_notification['id_compte'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, $bcc, []], '', false, 0, 0);
			...
			$ok = $send_now($email_adherent, $sujet, $html, $bcc);
			if (!$ok) {
				spip_log('Envoi direct validation_pre_paiement échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
		...
		$bcc = !empty($emails_adh) ? (count($emails_adh) == 1 ? $emails_adh[0] : $emails_adh) : false;
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - activation ' . intval($contexte_notification['id_compte'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, $bcc, []], '', false, 0, 0);
			...
			$ok = $send_now($email_adherent, $sujet, $html, $bcc);
			if (!$ok) {
				spip_log('Envoi direct activation échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
		...
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - justificatifs a revoir ' . intval($contexte_notification['id_compte'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
		...
		$contexte_notification['nb_jour_differences'] ??= 0;
		// Le contexte est désormais nettoyé en amont (merge contrôlé).
		$template = 'notifications/notification_echeances_adherent';
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - echeance_preventive ' . intval($contexte_notification['id_auteur'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
			...
			if (!$ok) {
				spip_log('Envoi direct echeance_preventive échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
		...
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - echeance_echu ' . intval($contexte_notification['id_auteur'] ?? 0), [$email_adherent, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
			...
			$ok = $send_now($email_adherent, $sujet, $html);
			if (!$ok) {
				spip_log('Envoi direct echeance_echu échoué pour ' . $email_adherent, 'association' . _LOG_ERREUR);
			}
			return $ok;
		}
	}
 ...
 */
function notifier_cotisation_preparer_contexte($query_cotisation = null, $query_categories = [], $query_transaction = [], $options = []) {
	...
	$query_cotisation = is_array($query_cotisation) ? $query_cotisation : [];
	$query_categories = is_array($query_categories) ? $query_categories : [];
	$query_transaction = is_array($query_transaction) ? $query_transaction : [];
	$options = is_array($options) ? $options : [];
	...
	$etat_justificatifs = ['total' => 0, 'valides' => 0, 'complet' => false, 'controle' => false];
	if ($id_compte > 0) {
...
function notifier_cotisation_admin($query_cotisation, $query_categories = [], $query_transaction = [], $type = '', $options = []) {
	...
	if ($type == 'attente_paiement') {
		...
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - attente_admin ' . intval($contexte_notification['id_compte'] ?? 0), [$emails_tres, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
			association_log('cotisations', 'Notification admin mise en file (attente_admin) pour ' . implode(',', (array) $emails_tres), 'info');
		} else {
			$html = recuperer_fond($template, $contexte_notification);
			$res = facteur_envoyer_app($emails_tres, $sujet, $html, false, ['use_queue' => $use_queue]);
			if (is_array($res) && empty($res['success'])) {
	...
	elseif ($type == 'attente_validation') {
		...
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - demande_admin ' . intval($contexte_notification['id_compte'] ?? 0), [$emails_tres, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
	...
	elseif ($type == 'encaissement_paiement') {
		...
		if ($use_queue) {
			job_queue_add('notifier_cotisation_envoyer', 'Notification - encaissement_admin ' . intval($contexte_notification['id_compte'] ?? 0), [$emails_tres, $sujet, $template, $contexte_notification, false, []], '', false, 0, 0);
...
function activer_adherent($id_auteur, $reinscription = '', $propager = true, $originateur = null) {
	...
	if ($propager && $compte_secondaire_activation === 'oui') {
		...
		if ($compte_principal && isset($compte_principal['auteur_compte_principal']) && intval($compte_principal['auteur_compte_principal']) > 0 &&
			...
			$comptes_secondaires = sql_allfetsel(
				'id_auteur',
				'spip_auteurs',
				'auteur_compte_principal=' . intval($id_auteur) . ' AND ' .
				 sql_in('statut_interne', ['prospect', 'echu', 'ok'])
			);
					...
					if ($id_secondaire > 0 && $id_secondaire != $id_auteur) {
						...
						if (empty($originateur) || intval($originateur) !== $id_secondaire) {
							try {
								...
								notifier_cotisation_adherent([], [], [], 'activation', ['id_auteur' => $id_secondaire, 'use_queue' => true]);
...
function inverser_relation_compte($id_secondaire) {
	...
	$r1 = sql_updateq('spip_auteurs', ['auteur_compte_principal' => null], 'id_auteur=' . $id_secondaire);
	...
	if (!empty($ids)) {
		foreach ($ids as $id) {
			...
			sql_updateq('spip_auteurs', ['auteur_compte_principal' => $id_secondaire], 'id_auteur=' . intval($id));
		}
	}

	// 3) Rattacher l'ancien principal au nouveau principal
	sql_updateq('spip_auteurs', ['auteur_compte_principal' => $id_secondaire], 'id_auteur=' . $id_principal);

	// Log après modification
	$comptes_apres = sql_allfetsel('id_auteur,auteur_compte_principal', 'spip_auteurs', 'id_auteur IN (' . implode(',', array_merge([$id_principal], $ids, [$id_secondaire])) . ')');
...
function obtenir_emails_tresoriers($type = null) {
	...
	if (is_array($parsed1)) {
		$list = array_merge($list, $parsed1);
	}
	$parsed2 = $parseur($meta_entreprise);
	if (is_array($parsed2)) {
		$list = array_merge($list, $parsed2);
	}
...
function association_mode_validite($type_adherent = 'adherent') {
	...
	if ($type_adherent === 'entreprise') {
		...
		if (in_array($validite_entreprise, ['annee', 'annuelle'], true)) {
			return 'annuelle';
		}

		$mode_entreprise = $metas['entreprise_validation_mode'] ?? '';
		if (in_array($mode_entreprise, ['scolaire', 'date_fixee', 'duree_mois', 'annee', 'annuelle'], true)) {
...
function association_contexte_adhesion($id_auteur, $date_reference = null) {
	...
	$champs_auteur = ['id_auteur', 'validite', 'statut_interne'];
	...
	include_spip('inc/association_adhesions_integrations');
	$transactions = association_adhesions_transactions_auteur_lire($id_auteur, ['commande', 'attente']);
	...
	} elseif (in_array($contexte['statut_interne'], ['echu', 'relance'], true)) {
		$contexte['contexte_inscription'] = 'reinscription';
