...
function association_adhesions_transactions_auteur_lire($id_auteur, $statuts = []) {
	if (!association_adhesions_integration_module_actif('association_paiements')) {
		return [];
	}
	include_spip('inc/association_paiements_transactions');
...
function association_adhesions_mailshot_creer($sujet, $html, array $destinataires, array $options = []) {
	include_spip('inc/association_capacites');
	$resultat = association_programmer_campagne([
		'sujet' => $sujet, 'html' => $html, 'destinataires' => $destinataires, 'options' => $options,
	]);
...
function association_adhesions_zones_options() {
	...
	$zones = [];
	foreach (sql_allfetsel('id_zone,titre', 'spip_zones', '', '', 'titre') ?: [] as $zone) {
