...
function association_adhesions_migration_legacy($version) {
	$operations = [
		...
		'1.2.2' => ['TABLE spip_asso_categories_adherents DROP deleted'],
		'1.3.3' => ["TABLE spip_asso_categories_adherents ADD COLUMN type_adherent varchar(255) NOT NULL DEFAULT 'adherent' AFTER statut"],
		'1.4.2' => ["TABLE spip_asso_categories_adherents ALTER type_adherent SET DEFAULT 'adherent'"],
		'1.5.0' => [
		...
		'1.5.3' => ["TABLE spip_asso_categories_adherents ADD COLUMN nombre_enfants VARCHAR(2) NULL DEFAULT ''"],
		'1.5.4' => [
		...
		'1.5.9' => ["TABLE spip_asso_categories_adherents ADD COLUMN devise VARCHAR(3) NOT NULL DEFAULT '' AFTER cotisation"],
	];
	foreach ($operations[(string) $version] ?? [] as $operation) {
		sql_alter($operation);
	}
	if ((string) $version === '1.1.0') {
		maj_tables(['spip_asso_categories']);
	}
	if (in_array((string) $version, ['1.1.3', '1.1.4', '1.1.5', '1.1.6', '1.1.9', '1.1.13'], true)) {
		association_import_champs_extras();
	}
	if (in_array((string) $version, ['1.1.7', '1.1.8'], true)) {
		maj_tables(['spip_asso_categories']);
	}
	if ((string) $version === '1.2.0') {
		maj_tables(['spip_asso_categories_adherents']);
	}
