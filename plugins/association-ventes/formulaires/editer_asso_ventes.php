...
function formulaires_editer_asso_ventes_verifier_dist($id_vente) {
	...
	if ($prix_vente < 0) {
		$erreurs['prix_vente'] = _T('association_ventes:erreur_montant');
	}
	if ($frais_envoi < 0) {
		$erreurs['frais_envoi'] = _T('association_ventes:erreur_montant');
	}
	if ($quantite < 0) {
		$erreurs['quantite'] = _T('association_ventes:erreur_montant');
	}
