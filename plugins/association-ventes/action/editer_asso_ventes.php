...
function action_editer_asso_ventes($id_vente = null) {
	return action_editer_asso_ventes_dist($id_vente);
}

function action_editer_asso_ventes_dist($id_vente = null) {
	if ($id_vente === null) {
	...
	if (!autoriser('modifier', 'vente', $id_vente)) {
		return [$id_vente, _T('info_acces_interdit')];
	}
...
function ventes_insert($date_vente, $article, $code, $acheteur, $id_acheteur, $quantite, $date_envoi, $frais_envoi, $prix_vente, $commentaire, $journal, $recette) {
	$id_vente = sql_insertq('spip_asso_ventes', [
		'date_vente' => $date_vente,
		'article' => $article,
		'code' => $code,
		'acheteur' => $acheteur,
